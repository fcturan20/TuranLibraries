...
enum class LOG_TYPE : unsigned char {
	CRASHING_ERROR = 0, SOME_ERROR = 1, WARNING = 2, STATUS = 3, NOT_CODEDPATH = 4
};
...
void WRITE_LOGs_toFILEs(const char* mainlogfile, const char* errorlogfile, const char* warninglogfile, const char* notcodedfile) {
    if(!mainlogfile){LOGSYS->MainLogFile_Path = mainlogfile;}
    if(!errorlogfile){LOGSYS->ErrorLogFile_Path = errorlogfile;}
    if(!warninglogfile){LOGSYS->WarningLogFile_Path = warninglogfile;}
    if(!notcodedfile){LOGSYS->NotCodedLogFile_Path = notcodedfile;}
