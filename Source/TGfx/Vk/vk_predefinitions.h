...
extern const struct TCProfiler* profilerSys;
...
#define VK_MEMOFFSET_TO_POINTER(offset) ((void*)(uintptr_t(VKCONST_VIRMEMSPACE_BEGIN) + uint32_t(offset)))
...
static_assert(sizeof(VKOBJHANDLE) == sizeof(void*),
...
static_assert(sizeof(VKDATAHANDLE) == sizeof(void*),
...
public:
	...
	bool isValid(T* obj)
	{
		...
		if (objINT < dataINT || (objINT - dataINT) % sizeof(T) || objINT >= dataINT + (sizeof(T) * maxObjIndx.load()))
		{
...
T* getOBJ(TGFXHND hnd)
{
	...
	if (handle.type != T::HANDLETYPE || handle.OBJ_memoffset == UINT32_MAX || handle.OBJ_memoffset == 0)
	{
		printer(result_tgfx_FAIL,
...
public:
	...
	{
		uintptr_t objINT = uintptr_t(obj), dataINT = uintptr_t(data);
		if (objINT < dataINT || (objINT - dataINT) % sizeof(T) || objINT >= dataINT + (sizeof(T) * count))
		{
			printer(result_tgfx_FAIL,
					"Given handle doesn't align with other elements in the array! You probably used a "
					"invalid arithmetic/write operations on the handle");
