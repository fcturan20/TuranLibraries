...
unsigned int GetThisThreadIndex() { return JOBSYS->ThreadIDs[std::this_thread::get_id()]; }
unsigned int GetThreadCount() { return std::thread::hardware_concurrency(); }
...
ECSPLUGIN_EXIT(ecssys, reloadFlag) { printf("Threaded job sys exit isn't coded"); }
/*
...
bool tapi_atomic_uint::LimitedSubtract_weak(const unsigned int& subtract, const unsigned int&
minlimit) { unsigned int x = data.load(); if (x - subtract < minlimit ||	//Subtraction is bigger x
- subtract > x				//UINT overflow ) { return false;
  }
...
void tapi_atomic_uint::LimitedSubtract_strong(const unsigned int& subtract, const unsigned int&
minlimit) { while (true) { if (LimitedSubtract_weak(subtract, minlimit)) { return;
    }
    std::this_thread::yield();
  }
}
