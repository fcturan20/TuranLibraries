...
struct framegraphsys_vk {
  ...
  // Each submission should have current frame dependencies (normal passes + WP) + 1 (for next
  // frame, but not supported in rendergraph_primitive) number of semaphores
...
inline static void find_and_clear_swapchainfence(WINDOW_VKOBJ* window) {
  ...
  { if (fence.windows[window_i] == window) { for (unsigned int semaphore_i = 0; semaphore_i <
  fence.semaphores.size(); semaphore_i++) {
...
void Allocate_finalWaitMemory() {
  if (framegraphsys_vk::WAITs_DYNAMICMEM) {
    vk_virmem::free_dynamicmem(framegraphsys_vk::WAITs_DYNAMICMEM);
  }
  framegraphsys_vk::WAITs_DYNAMICMEM =
    vk_virmem::allocate_dynamicmem(framegraphsys_vk::WAITs_DYNAMICMEM_PAGECOUNT);
}
...
/*
void SetBarrier_BetweenPasses(VkCommandBuffer CB, VK_PassDesc* CurrentPass, VK_PassDesc* LastPass,
VkPipelineStageFlags& srcPipelineStage) { printer(result_tgfx_WARNING, "SetBarrier_BetweenPasses()
isn't coded");
}
void Record_RenderPass(VkCommandBuffer CB, RenderGraph::DP_VK* DrawPass) {
  ...
  rtslots_vk& CF_SLOTs =
contentmanager->GETRTSLOTSET_ARRAY().getOBJbyINDEX(DrawPass->BASESLOTSET_ID)->PERFRAME_SLOTSETs[FRAMEINDEX];
  ...
  std::vector<VkClearValue> CLEARVALUEs(CF_SLOTs.COLORSLOTs_COUNT + (CF_SLOTs.DEPTHSTENCIL_SLOT ? 1
: 0)); if (CF_SLOTs.COLORSLOTs_COUNT) { for (unsigned char COLORRTIndex = 0; COLORRTIndex <
CF_SLOTs.COLORSLOTs_COUNT; COLORRTIndex++) { CLEARVALUEs[COLORRTIndex] = {
CF_SLOTs.COLOR_SLOTs[COLORRTIndex].CLEAR_COLOR.x, CF_SLOTs.COLOR_SLOTs[COLORRTIndex].CLEAR_COLOR.y,
  ...
  for (unsigned char SubPassIndex = 0; SubPassIndex < DrawPass->ALLSubDPsCOUNT; SubPassIndex++) {
    ...
    if (SP.RG_BINDINDEX != SubPassIndex) {
      printer(result_tgfx_FAIL, "Subpass Binding Index and the SubDrawPass' element index doesn't
match, handle this case!");
    }
    //NonIndexed Draw Calls
    {
      VkDescriptorSet Sets[VKCONST_MAXDESCSET_PERLIST * 2];
      for (unsigned char DrawCallIndex = 0; DrawCallIndex < SP.NonIndexedDrawCalls.size();
DrawCallIndex++) { RenderGraph::nonindexeddrawcall_vk& DrawCall =
SP.NonIndexedDrawCalls[DrawCallIndex]; VkViewport DefaultViewport = {}; DefaultViewport.height =
(float)DrawPass->RenderRegion.HEIGHT; DefaultViewport.maxDepth = 1.0f; DefaultViewport.minDepth =
0.0f; DefaultViewport.width = (float)DrawPass->RenderRegion.WIDTH; DefaultViewport.x =
(float)DrawPass->RenderRegion.XOffset; DefaultViewport.y = (float)DrawPass->RenderRegion.YOffset;
        ...
        for (uint32_t i = 0; i < VKCONST_MAXDESCSET_PERLIST && DrawCall.base.SETs[i] != UINT16_MAX;
i++, bindingtablelist_size++) { VkDescriptorSet set =
contentmanager->GETDESCSET_ARRAY().getOBJbyINDEX(DrawCall.base.SETs[i])->Sets[VKGLOBAL_FRAMEINDEX];
          if (set == Sets[i] && offset == i) { offset++; continue; }
          Sets[i] = set;
        }
        vkCmdBindDescriptorSets(CB, VK_PIPELINE_BIND_POINT_COMPUTE, DrawCall.base.MatTypeLayout,
offset, bindingtablelist_size - offset, Sets, 0, nullptr); if (DrawCall.VBuffer != VK_NULL_HANDLE) {
          vkCmdBindVertexBuffers(CB, 0, 1, &DrawCall.VBuffer, &DrawCall.VOffset);
        }
        vkCmdDraw(CB, DrawCall.VertexCount, DrawCall.InstanceCount, DrawCall.FirstVertex,
DrawCall.FirstInstance);
    ...
    {
      VkDescriptorSet Sets[VKCONST_MAXDESCSET_PERLIST * 2];
      for (unsigned char DrawCallIndex = 0; DrawCallIndex < SP.IndexedDrawCalls.size();
DrawCallIndex++) { RenderGraph::indexeddrawcall_vk& DrawCall = SP.IndexedDrawCalls[DrawCallIndex];
...
offset, bindingtablelist_size - offset, Sets, 0, nullptr); if (DrawCall.VBuffer != VK_NULL_HANDLE) {
        ...
        vkCmdDrawIndexed(CB, DrawCall.IndexCount, DrawCall.InstanceCount, DrawCall.FirstIndex,
DrawCall.VOffset, DrawCall.FirstInstance);
      }
...
void Record_BarrierTP(VkCommandBuffer CB, VkPipelineStageFlags srcPipelineStage,
RenderGraph::SubTPBARRIER_VK* subpass) { VkImageMemoryBarrier* IMBARRIERS =
(VkImageMemoryBarrier*)VK_ALLOCATE_AND_GETPTR(VKGLOBAL_VIRMEM_CURRENTFRAME,
sizeof(VkImageMemoryBarrier) * subpass->TextureBarriers.size()); for (unsigned int i = 0; i <
subpass->TextureBarriers.size(); i++) { const RenderGraph::VK_ImBarrierInfo& info =
subpass->TextureBarriers[i]; VkImageMemoryBarrier& Barrier = IMBARRIERS[i];
    ...
    Barrier.subresourceRange.aspectMask = (info.isColorBit ? VK_IMAGE_ASPECT_COLOR_BIT : 0) |
(info.isDepthBit ? VK_IMAGE_ASPECT_DEPTH_BIT : 0) | (info.isStencilBit ? VK_IMAGE_ASPECT_STENCIL_BIT
: 0);
...
Find_TextureLayer_fromtgfx_cubeface(info.cubemapface);
    }
  }


  VkBufferMemoryBarrier* BUFBARRIERS =
(VkBufferMemoryBarrier*)VK_ALLOCATE_AND_GETPTR(VKGLOBAL_VIRMEM_CURRENTFRAME,
sizeof(VkBufferMemoryBarrier) * subpass->BufferBarriers.size()); for (unsigned int i = 0; i <
subpass->BufferBarriers.size(); i++) { const RenderGraph::VK_BufBarrierInfo& info =
subpass->BufferBarriers[i]; VkBufferMemoryBarrier& Barrier = BUFBARRIERS[i]; Barrier.buffer =
info.buf; Barrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED; Barrier.offset = info.offset;
    Barrier.pNext = nullptr;
  ...
  vkCmdPipelineBarrier(CB, srcPipelineStage, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
VK_DEPENDENCY_BY_REGION_BIT, 0, nullptr, 0, nullptr, subpass->TextureBarriers.size(), IMBARRIERS);
}
...
void Record_CopyTP(VkCommandBuffer CB, RenderGraph::SubTPCOPY_VK* subpass) {
  ...
  for (unsigned int i = 0; i < subpass->BUFIMCopies.size(); i++) {
    ...
    vkCmdCopyBufferToImage(CB, info.SourceBuffer, info.TargetImage,
VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &info.BufferImageCopy);
  }
  for (unsigned int i = 0; i < subpass->IMIMCopies.size(); i++) {
    RenderGraph::VK_IMtoIMinfo& info = subpass->IMIMCopies[i];
    vkCmdCopyImage(CB, info.SourceTexture, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, info.TargetTexture,
VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &info.info);
  }
}
...
void Record_ComputePass(VkCommandBuffer CB, RenderGraph::CP_VK* CP) {
  ...
  for (unsigned int SPIndex = 0; SPIndex < CP->SubPassesCount; SPIndex++) {
    ...
    for (unsigned int DispatchIndex = 0; DispatchIndex < SP.Dispatches.size(); DispatchIndex++) {
      ...
      uint32_t offset = 0, bindingtablelist_size = 0;
      for (uint32_t i = 0; i < VKCONST_MAXDESCSET_PERLIST && dc.BINDINGTABLEIDs[i] != UINT16_MAX;
i++, bindingtablelist_size++) { VkDescriptorSet set =
contentmanager->GETDESCSET_ARRAY().getOBJbyINDEX(dc.BINDINGTABLEIDs[i])->Sets[VKGLOBAL_FRAMEINDEX];
        if (set == Sets[i] && offset == i) { offset++; continue; }
        Sets[i] = set;
      }
      vkCmdBindDescriptorSets(CB, VK_PIPELINE_BIND_POINT_COMPUTE, dc.Layout, offset,
bindingtablelist_size - offset, Sets, 0, nullptr); vkCmdDispatch(CB, dc.DispatchSize.x,
dc.DispatchSize.y, dc.DispatchSize.z);
    }
  }
}
...
result_tgfx Execute_RenderGraph() {
  unsigned char FrameIndex = renderer->Get_FrameIndex(false);
  commandbuffer_vk* cb = queuesys->get_commandbuffer(rendergpu, rendergpu->GRAPHICSQUEUEFAM(),
renderer->Get_FrameIndex(false)); VkCommandBuffer CB = queuesys->get_commandbufferobj(cb);
  ...
  for (unsigned int pass_i = 0; pass_i < framegraphsys_vk::Execution_Order.size(); pass_i++) {
    ...
    if (pass_i > 0) {
      SetBarrier_BetweenPasses(CB, framegraphsys_vk::Execution_Order[pass_i],
framegraphsys_vk::Execution_Order[pass_i - 1], srcPipelineStage);
    }
    ...
    case VK_PassDesc::PassType::WP:
      if (pass_i != framegraphsys_vk::Execution_Order.size() - 1) {
        printer(result_tgfx_FAIL, "Primitive Rendergraph supports windows passes only to be in last
in the execution order, please fix your execution order!");
      }
      break;
  ...
  //Before sending render command buffers, wait for presentation of current frame (that has the
penultimate frame's content) to end
  ...
  //Find how many display queues will be used (to detect how many semaphores render command buffers
should signal) struct PresentationInfos { std::vector<VkSwapchainKHR> swapchains;
  ...
  for (unsigned char i = 0; i < 1; i++) {
    RenderGraph::WP_VK* WP = (RenderGraph::WP_VK*)VKGLOBAL_RG->GetNextPass(nullptr,
VK_PASSTYPE::WP); if (!WP->WindowCalls.size()) { continue;
    }
    for (unsigned char WindowIndex = 0; WindowIndex < WP->WindowCalls.size(); WindowIndex++) {
      ...
      bool is_found = false;
      for (unsigned char queue_i = 0; queue_i < PresentationInfo_PerQueue.size() && !is_found;
queue_i++) { PresentationInfos& presentation = PresentationInfo_PerQueue[queue_i]; if (window_qfam
== presentation.queuefam) { presentation.image_indexes.push_back(window->CurrentFrameSWPCHNIndex);
      ...
      if (!is_found) {
        ...
        PresentationInfos& presentation = PresentationInfo_PerQueue[PresentationInfo_PerQueue.size()
- 1]; presentation.image_indexes.push_back(window->CurrentFrameSWPCHNIndex);
        window->CurrentFrameSWPCHNIndex = (window->CurrentFrameSWPCHNIndex + 1) % 2;
        presentation.swapchains.push_back(window->Window_SwapChain);
        presentation.windows.push_back(window);
  ...
  framegraphsys_vk::RENDERING_FENCEs[FrameIndex] = { queuesys->queueSubmit(rendergpu,
rendergpu->GRAPHICSQUEUEFAM(), SwapchainAcquires, signalSemaphores, &CB, &waitdststagemask, 1) };
  ...
  for (unsigned char i = 0; i < 1; i++) {
    ...
    for (unsigned char presentation_i = 0; presentation_i < PresentationInfo_PerQueue.size();
presentation_i++) { VkResult result; VkPresentInfoKHR SwapchainImage_PresentationInfo = {};
      SwapchainImage_PresentationInfo.sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR;
      SwapchainImage_PresentationInfo.pNext = nullptr;
      SwapchainImage_PresentationInfo.swapchainCount =
PresentationInfo_PerQueue[presentation_i].swapchains.size();
      SwapchainImage_PresentationInfo.pSwapchains =
PresentationInfo_PerQueue[presentation_i].swapchains.data();
      SwapchainImage_PresentationInfo.pImageIndices =
PresentationInfo_PerQueue[presentation_i].image_indexes.data();
      ...
      for (unsigned int signalSemaphoreIndex = 0; signalSemaphoreIndex < signalSemaphores_vk.size();
signalSemaphoreIndex++) { signalSemaphores_vk[signalSemaphoreIndex] =
semaphoresys->GetSemaphore_byID(signalSemaphores[signalSemaphoreIndex]).vksemaphore();
      }
      ...
      if (vkQueuePresentKHR(queuesys->get_queue(rendergpu,
PresentationInfo_PerQueue[presentation_i].queuefam), &SwapchainImage_PresentationInfo) !=
VK_SUCCESS) { printer(result_tgfx_FAIL, "Submitting Presentation Queue has failed!"); return
result_tgfx_FAIL;
      }
    }
  }
...
VK_PASS_ID_TYPE find_mergedpass(passwaitdescription_tgfx_listhandle waitlist) {
  ...
  for (uint32_t i = 0; i < waitlistsize; i++) {
    ...
    switch (handle.type) {
      case VKHANDLETYPEs::WAITDESC_SUBCP: found_passtype = VK_PASSTYPE::CP; break;
      case VKHANDLETYPEs::WAITDESC_SUBDP: found_passtype = VK_PASSTYPE::DP; break;
      case VKHANDLETYPEs::WAITDESC_SUBTP: found_passtype = VK_PASSTYPE::TP; break;
      default: break;
    }
...
//------------------
void ConvertWaits(uint32_t& PassWaitsPTR, uint16_t& PassWaitCount,
                  passwaitdescription_tgfx_listhandle passwaits) {
  PassWaitsPTR  = 0;
  PassWaitCount = 0;
  ...
  for (uint32_t wait_i = 0; wait_i < waitlistsize; wait_i++) {
    ...
    switch (waithandle.type) {
      ...
      case VKHANDLETYPEs::WAITDESC_SUBDP:
        passwaitPTR = RenderGraph::getDesc_fromHandle<RenderGraph::WaitDesc_SubDP>(waithandle)
                        ->convert_to_FinalWaitDesc();
        PassWaitCount++;
        break;
      case VKHANDLETYPEs::WAITDESC_SUBTP:
        passwaitPTR = RenderGraph::getDesc_fromHandle<RenderGraph::WaitDesc_SubTP>(waithandle)
                        ->convert_to_FinalWaitDesc();
        PassWaitCount++;
        break;
    ...
    if (!PassWaitsPTR) {
      PassWaitsPTR = passwaitPTR;
    }
  }
}
void ConvertSubpassWaits(passwaitdescription_tgfx_listhandle passwaits, VKHANDLETYPEs passwaittype,
...
void RenderGraph::CP_VK::convert_waits() {
  ...
  for (uint32_t i = 1; i < base.ALLSUBPASSESCOUNT; i++) {
    ...
    if (MERGEDDP_ID != VK_PASS_INVALID_ID && MERGEDDP_ID != dp_id) {
      printer(20);
      return;
    }
    MERGEDDP_ID = dp_id;
    ...
    if (MERGEDTP_ID != VK_PASS_INVALID_ID && MERGEDTP_ID != tp_id) {
      printer(20);
      return;
    }
    MERGEDTP_ID = tp_id;
...
void RenderGraph::TP_VK::convert_waits() {
  ...
  for (subtp_i; subtp_i < base.ALLSUBPASSESCOUNT; subtp_i++) {
    ...
    if (subpass->TYPE == VK_PASSTYPE::SUBTP_BARRIER || subpass->TYPE == VK_PASSTYPE::SUBTP_COPY) {
      ...
      ConvertSubpassWaits(waits, VKHANDLETYPEs::WAITDESC_SUBDP, VK_POINTER_TO_MEMOFFSET(this));
      if (subpass->TYPE == VK_PASSTYPE::SUBTP_BARRIER) {
        subtp = (( char* )subtp) + sizeof(RenderGraph::SubTPBARRIER_VK);
        subtp_i++;
      }
      if (subpass->TYPE == VK_PASSTYPE::SUBTP_COPY) {
        subtp = (( char* )subtp) + sizeof(RenderGraph::SubTPCOPY_VK);
        subtp_i++;
      }

      VK_PASS_ID_TYPE cp_id = find_mergedpass<RenderGraph::CP_VK>(waits);
      if (MERGEDCP_ID != VK_PASS_INVALID_ID && MERGEDCP_ID != cp_id) {
        printer(20);
        return;
      }
      MERGEDCP_ID = cp_id;
...
uint32_t convert_waitdesc_toSubPassWait(VKDATAHANDLE deschandle) {
  ...
  WaitDescStruct::TARGETPASS_STRUCT* targetpass =
    ( WaitDescStruct::TARGETPASS_STRUCT* )VK_MEMOFFSET_TO_POINTER(
      target_mainpasshandle.OBJ_memoffset);
  if (targetcp->base.PASSTYPE != VK_PASSTYPE::DP) {
    printer(17);
    return UINT32_MAX;
  }
  if (targetcp->base.ALLSUBPASSESCOUNT <= SubPassIndex) {
    printer(8);
    return UINT32_MAX;
  }
#endif
  uint32_t passwaitPTR = vk_virmem::allocate_from_dynamicmem(framegraphsys_vk::WAITs_DYNAMICMEM,
                                                             sizeof(RenderGraph::WaitDescFinal));
  RenderGraph::SubPassCOMMON::SubPassWait* finaldesc =
    ( RenderGraph::SubPassCOMMON::SubPassWait* )VK_MEMOFFSET_TO_POINTER(passwaitPTR);
...
uint32_t RenderGraph::WaitDesc_SubDP::convert_to_FinalWaitDesc() {
  ...
  if (target_mainpasshandle.type != VKHANDLETYPEs::DRAWPASS) {
    printer(17);
    return UINT32_MAX;
  }
  RenderGraph::DP_VK* targetcp =
    ( RenderGraph::DP_VK* )VK_MEMOFFSET_TO_POINTER(target_mainpasshandle.OBJ_memoffset);
  if (targetcp->base.PASSTYPE != VK_PASSTYPE::DP) {
    printer(17);
    return UINT32_MAX;
  }
  ...
  uint32_t passwaitPTR = vk_virmem::allocate_from_dynamicmem(framegraphsys_vk::WAITs_DYNAMICMEM,
                                                             sizeof(RenderGraph::WaitDescFinal));
  RenderGraph::WaitDescFinal* finaldesc =
    ( RenderGraph::WaitDescFinal* )VK_MEMOFFSET_TO_POINTER(passwaitPTR);
...
#ifdef VULKAN_DEBUGGING
  VKOBJHANDLE target_mainpasshandle = (*( VKOBJHANDLE* )passhandle);
  if (target_mainpasshandle.type != VKHANDLETYPEs::COMPUTEPASS) {
    printer(17);
    return UINT32_MAX;
  }
  RenderGraph::CP_VK* targetcp =
    ( RenderGraph::CP_VK* )VK_MEMOFFSET_TO_POINTER(target_mainpasshandle.OBJ_memoffset);
  if (targetcp->base.PASSTYPE != VK_PASSTYPE::CP) {
    printer(17);
    return UINT32_MAX;
  }
  if (targetcp->base.ALLSUBPASSESCOUNT <= SubPassIndex) {
...
#ifdef VULKAN_DEBUGGING
  VKOBJHANDLE target_mainpasshandle = (*( VKOBJHANDLE* )passhandle);
  if (target_mainpasshandle.type != VKHANDLETYPEs::TRANSFERPASS) {
    printer(17);
    return UINT32_MAX;
  }
  RenderGraph::TP_VK* targetcp =
    ( RenderGraph::TP_VK* )VK_MEMOFFSET_TO_POINTER(target_mainpasshandle.OBJ_memoffset);
  if (targetcp->base.PASSTYPE != VK_PASSTYPE::TP) {
    printer(17);
    return UINT32_MAX;
  }
  if (targetcp->base.ALLSUBPASSESCOUNT <= SubPassIndex) {
