...
void GLFWwindowresizecallback(GLFWwindow* glfwwindow, int width, int height)
{
	...
	vkwindow->resize_cb((struct tgfx_window*)vkwindow,
						vkwindow->UserPTR,
						width,
						height,
						(struct tgfx_texture**)vkwindow->Swapchain_Textures);
}
bool Create_WindowSwapchain(window_dx* new_window,
							unsigned int WIDTH,
							unsigned int HEIGHT,
...
public:
	...
	// supported for now.
	static void createwindow(unsigned int WIDTH,
							 unsigned int HEIGHT,
													  ...
													  tgfx_PrintLogCallback printcallback)
{
	...
	THREADINGSYS_TAPI_PLUGIN_LOAD_TYPE threadsysloaded = (THREADINGSYS_TAPI_PLUGIN_LOAD_TYPE)regsys->get(
		THREADINGSYS_TAPI_PLUGIN_NAME, THREADINGSYS_TAPI_PLUGIN_VERSION, 0);
	if (threadsysloaded)
	{
		gThreadingSystem = threadsysloaded->funcs;
	}

	gCoreDx = new core_public;
	core->data = (core_tgfx_d*)gCoreDx;
	gCoreTgfxMain = core->api;
	...
	core_functions_dx::createwindow(
		1280, 720, nullptr, windowmode_tgfx_WINDOWED, "First Window", nullptr, nullptr, nullptr, nullptr, &firstwindow);
}

void core_functions_dx::createwindow(unsigned int WIDTH,
									 unsigned int HEIGHT,
									 struct tgfx_monitor* monitor,
									 windowmode_tgfx Mode,
									 const char* NAME,
									 ...
									 struct tgfx_window** window)
{
	...
	GLFWwindow* glfw_window = glfwCreateWindow(
		WIDTH, HEIGHT, NAME, (Mode == windowmode_tgfx_FULLSCREEN) ? (MONITOR->monitorobj) : (NULL), nullptr);
