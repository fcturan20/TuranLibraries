...
TCORE_PLUGIN_DEFINE(TSUT, "tcUnitTestSystem", TCUnitTest, TCORE_MAKE_PLUGIN_VERSION(0, 0, 0))
...
typedef struct TCUnitTestDescription {
  ...
  // So, if a plugin named "MyPlugin" registers a unit test named "MyTest", the full name of the unit test will be "MyPlugin.MyTest". This ensures that unit test names are unique across different plugins.
  const char* Name;
  ...
  unsigned char (*Test)(TCReadBuffer inputData);
} TCUnitTestDescription;

typedef struct TCUnitTest {
  ...
  // If inputData is not empty, it will be passed to the test function. Otherwise default data will be used.
  // If the test returns 0, it is successful. Otherwise, it is failed.
