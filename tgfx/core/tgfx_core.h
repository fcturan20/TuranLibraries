...
typedef struct tgfx_core{
	...
	result_tgfx (*load_backend)
		(tgfx_core* parent, backends_tgfx backend, tgfx_PrintLogCallback printFnc);
	void (*initialize_secondstage)(initSecondStageInfo_tgfxhnd info);
	// Also create a supported swapchain with create_swapchain() to use the window
	// Create a new swapchain in the callback too
	void (*create_window)
	(
		unsigned int WIDTH, unsigned int HEIGHT, monitor_tgfxhnd monitor, 
		windowmode_tgfx Mode, const char* NAME, tgfx_windowResizeCallback ResizeCB, 
		void* UserPointer, window_tgfxhnd* window
	 );
	// @param textures: Should point to an array of swapchainImageCount elements!
	result_tgfx(*create_swapchain)
	(
		window_tgfxhnd window, windowpresentation_tgfx presentationMode,
		windowcomposition_tgfx composition, colorspace_tgfx colorSpace,
		textureChannels_tgfx channels, textureUsageFlag_tgfxhnd SwapchainUsage,
		texture_tgfxhnd* textures
	);
	void (*change_window_resolution)
		(window_tgfxhnd WindowHandle, unsigned int width, unsigned int height);
	...
	void (*debugcallback_threaded)
		(unsigned char ThreadIndex, result_tgfx Result, const char* Text);
...
#define TGFXLISTCOUNT(gfxcoreptr, listobj, countername) \
	...
	while (listobj[countername] != gfxcoreptr->INVALIDHANDLE) { countername++; }

typedef struct tapi_ecs ecs_tapi;
//This function should be exported by the backend dll
typedef result_tgfx (*backend_load_func)
	(ecs_tapi* regsys, core_tgfx_type* core, tgfx_PrintLogCallback printcallback);