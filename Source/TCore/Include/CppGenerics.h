...
// Use this right after a type class definition to automatically register the object to
// GetOpaqueHandle() and GetFromOpaqueHandle() overloads.
#define TCORE_DEFINE_HANDLE_TYPE_CONVERTERS(TcObjectTypeName, SystemPrefix)                     \
	typedef decltype(TcObjectTypeName::GetOpaqueHandle(nullptr))                                \
		TcObjectTypeName##_TCOpaqueHandleType;                                                  \
                                                                                               \
	inline TcObjectTypeName* Get##SystemPrefix##Object(                                         \
		TcObjectTypeName##_TCOpaqueHandleType opaqueHandle)                                     \
	{                                                                                          \
		return TcObjectTypeName::Get##SystemPrefix##Object(opaqueHandle);                       \
	}                                                                                          \
                                                                                               \
	inline TcObjectTypeName##_TCOpaqueHandleType GetOpaqueHandle(                              \
		TcObjectTypeName* object)                                                               \
	{                                                                                          \
