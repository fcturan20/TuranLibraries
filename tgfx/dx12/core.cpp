...
void GLFWwindowresizecallback(GLFWwindow* glfwwindow, int width, int height) {
  ...
  vkwindow->resize_cb(( struct tgfx_window* )vkwindow, vkwindow->UserPTR, width, height,
                      ( struct tgfx_texture** )vkwindow->Swapchain_Textures);
}
bool Create_WindowSwapchain(window_dx* new_window, unsigned int WIDTH, unsigned int HEIGHT,
                            ComPtr<IDXGISwapChain4>* SwapchainOBJ,
                            texture_dx**             SwapchainTextureHandles) {
  ...
  ThrowIfFailed(dxgiFactory->CreateSwapChainForHwnd(commandQueue.Get(), new_window->Window_Handle,
                                                    &swapChainDesc, nullptr, nullptr, &swapChain1));
  ThrowIfFailed(
    dxgiFactory->MakeWindowAssociation(new_window->Window_Handle, DXGI_MWA_NO_ALT_ENTER));
...
struct core_functions_dx {
 public:
  // SwapchainTextureHandles should point to an array of 2 elements! Triple Buffering is not
  // supported for now.
  static void createwindow(unsigned int WIDTH, unsigned int HEIGHT, struct tgfx_monitor* monitor,
                           windowmode_tgfx Mode, const char* NAME,
                           textureUsageFlag_tgfxhnd  SwapchainUsage,
                           tgfx_windowResizeCallback ResizeCB, void* UserPointer,
                           struct tgfx_texture** SwapchainTextureHandles, struct tgfx_window** window);
  static void change_window_resolution(struct tgfx_window* WindowHandle, unsigned int width,
                                       unsigned int height);
  static void getmonitorlist(monitor_tgfxlsthnd* MonitorList);
  static void getGPUlist(gpu_tgfxlsthnd* GpuList);

  // Debug callbacks are user defined callbacks, you should assign the function pointer if you want
  // to use them As default, all backends set them as empty no-work functions
  static void debugcallback(result_tgfx result, const char* Text);
  // You can set this if TGFX is started with threaded call.
  static void debugcallback_threaded(unsigned char ThreadIndex, result_tgfx Result,
                                     const char* Text);
...
extern "C" FUNC_DLIB_EXPORT result_tgfx backend_load(registrysys_tapi* regsys, core_tgfx_type* core,
  ...
  core->api->INVALIDHANDLE =
    regsys->get(VIRTUALMEMORY_TAPI_PLUGIN_NAME, VIRTUALMEMORY_TAPI_PLUGIN_VERSION, 0);
  // Check if threading system is loaded
  THREADINGSYS_TAPI_PLUGIN_LOAD_TYPE threadsysloaded =
    ( THREADINGSYS_TAPI_PLUGIN_LOAD_TYPE )regsys->get(THREADINGSYS_TAPI_PLUGIN_NAME,
                                                      THREADINGSYS_TAPI_PLUGIN_VERSION, 0);
  if (threadsysloaded) {
  ...
  struct tgfx_window* firstwindow;
  core_functions_dx::createwindow(1280, 720, nullptr, windowmode_tgfx_WINDOWED, "First Window",
                                  nullptr, nullptr, nullptr, nullptr, &firstwindow);
}

void core_functions_dx::createwindow(unsigned int WIDTH, unsigned int HEIGHT,
                                     struct tgfx_monitor* monitor, windowmode_tgfx Mode,
                                     const char* NAME, textureUsageFlag_tgfxhnd SwapchainUsage,
                                     tgfx_windowResizeCallback ResizeCB, void* UserPointer,
  ...
  GLFWwindow* glfw_window = glfwCreateWindow(
    WIDTH, HEIGHT, NAME, (Mode == windowmode_tgfx_FULLSCREEN) ? (MONITOR->monitorobj) : (NULL),
    nullptr);
  ...
  if (!Create_WindowSwapchain(new_window, new_window->LASTWIDTH, new_window->LASTHEIGHT,
                              &new_window->Swapchain_Handle, SWPCHNTEXTUREHANDLESVK)) {
  ...
  SwapchainTextureHandles[0]        = ( struct tgfx_texture* )new_window->Swapchain_Textures[0];
  SwapchainTextureHandles[1]        = ( struct tgfx_texture* )new_window->Swapchain_Textures[1];

  *window = ( struct tgfx_window* )new_window;
}
...
void core_functions_dx::Check_Computer_Specs() {
  ...
  printer(result_tgfx_WARNING,
          "D3D12 backend only uses the GPU with highest Dedicated Video Memory size right now!");
  ...
  for (UINT i = 0; dxgiFactory->EnumAdapters1(i, &currentAdapter) != DXGI_ERROR_NOT_FOUND; ++i) {
    ...
    if (!SUCCEEDED(D3D12CreateDevice(currentAdapter.Get(), D3D_FEATURE_LEVEL_12_0,
                                     __uuidof(ID3D12Device), nullptr))) {
      printer(result_tgfx_FAIL,
              "Current GPU doesn't support D3D_FEATURE_LEVEL_12_0 so skipped it.");
      continue;
    }
  ...
  ThrowIfFailed(D3D12CreateDevice(finalAdapter.Get(), D3D_FEATURE_LEVEL_11_0,
                                  IID_PPV_ARGS(&finalLogicalDevice)));
  ...
  if (SUCCEEDED(finalLogicalDevice.As(&pInfoQueue))) {
    ...
    D3D12_MESSAGE_ID DenyIds[] = {
      D3D12_MESSAGE_ID_CLEARRENDERTARGETVIEW_MISMATCHINGCLEARVALUE, // I'm really not sure how to
                                                                    // avoid this message.
      D3D12_MESSAGE_ID_MAP_INVALID_NULLRANGE, // This warning occurs when using capture frame while
                                              // graphics debugging.
      D3D12_MESSAGE_ID_UNMAP_INVALID_NULLRANGE, // This warning occurs when using capture frame
                                                // while graphics debugging.
    };
