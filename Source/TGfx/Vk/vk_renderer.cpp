...
struct vkCmdStruct_bindBindingTables
{
	...
	void cmd_execute(VkCommandBuffer cb, CMDBUNDLE_VKOBJ* cmdBundle)
	{
		...
		vkCmdBindDescriptorSets(
			cb, vk_bindPoint, cmdBundle->vk_activePipelineLayout, m_firstSetIndx, m_setCount, sets, 0, nullptr);
	}
...
struct vkCmdStruct_setViewport
{
	...
	void cmd_execute(VkCommandBuffer cb, CMDBUNDLE_VKOBJ* cmdBundle) { vkCmdSetViewport(cb, 0, 1, &vk_viewport); };
...
struct vkCmdStruct_setScissor
{
	...
	void cmd_execute(VkCommandBuffer cb, CMDBUNDLE_VKOBJ* cmdBundle) { vkCmdSetScissor(cb, 0, 1, &vk_rect); };
	VkRect2D vk_rect = {};
};
...
struct vkCmdStruct_executeIndirect
{
	...
	void cmd_execute(VkCommandBuffer cb, CMDBUNDLE_VKOBJ* cmdBundle)
	{
		...
		for (uint32_t stateIndx = 0; stateIndx < opStateCount; stateIndx++)
		{
			...
			uint64_t indirectArgumentDataSize = vk_findIndirectOperationDataSize(opStates[stateIndx].opType);
			...
			for (uint32_t loopIndx = 0; loopIndx < loopCount; loopIndx++)
			{
				...
				case indirectOperationType_tgfx_DRAWINDEXED:
					vkCmdDrawIndexedIndirect(cb, vk_buffer, activeOffset, drawCount, indirectArgumentDataSize);
					break;
				case indirectOperationType_tgfx_DISPATCH: vkCmdDispatchIndirect(cb, vk_buffer, activeOffset); break;
...
struct vkCmdStruct_pushConstant
{
	...
	void cmd_execute(VkCommandBuffer cb, CMDBUNDLE_VKOBJ* cmdBundle)
	{
		vkCmdPushConstants(cb, cmdBundle->vk_activePipelineLayout, VK_SHADER_STAGE_ALL, offset, size, data);
	};
...
void vk_executeCmd(VkCommandBuffer cb, CMDBUNDLE_VKOBJ* bundle, const vk_cmd& cmd)
{
	switch (cmd.cmd_type)
	{
	case vk_cmdType::barrierTexture: ((vkCmdStruct_barrierTexture*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::bindBindingTables: ((vkCmdStruct_bindBindingTables*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::bindPipeline: ((vkCmdStruct_bindPipeline*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::dispatch: ((vkCmdStruct_dispatch*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::setScissor: ((vkCmdStruct_setScissor*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::setViewport: ((vkCmdStruct_setViewport*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::drawNonIndexedDirect:
		((vkCmdStruct_drawNonIndexedDirect*)cmd.cmd_data)->cmd_execute(cb, bundle);
		break;
	case vk_cmdType::setDepthBounds: ((vkCmdStruct_setDepthBounds*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::copyBufferToTexture:
		((vkCmdStruct_copyBufferToTexture*)cmd.cmd_data)->cmd_execute(cb, bundle);
		break;
	case vk_cmdType::bindVertexBuffers: ((vkCmdStruct_bindVertexBuffers*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::bindIndexBuffer: ((vkCmdStruct_bindIndexBuffer*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::drawIndexedDirect: ((vkCmdStruct_drawIndexedDirect*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::executeIndirect: ((vkCmdStruct_executeIndirect*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
	case vk_cmdType::copyBufferToBuffer:
		((vkCmdStruct_copyBufferToBuffer*)cmd.cmd_data)->cmd_execute(cb, bundle);
		break;
	case vk_cmdType::pushConstant: ((vkCmdStruct_pushConstant*)cmd.cmd_data)->cmd_execute(cb, bundle); break;
...
void vk_destroyCmd(vk_cmd& cmd)
{
	switch (cmd.cmd_type)
	{
	case vk_cmdType::executeIndirect: ((vkCmdStruct_executeIndirect*)cmd.cmd_data)->cmd_destroy(); break;
	}
}
...
void vk_createFences(struct tgfx_gpu* gpu, unsigned int count, unsigned int initValue, struct tgfx_fence** fenceList)
{
...
void vk_finishCommandBundle(struct tgfx_commandBundle* bndl, unsigned int extCount, struct tgfx_extension* const* exts)
{
...
void vk_cmdBindBindingTables(struct tgfx_commandBundle* bndl,
							 ...
							 pipelineType_tgfx pipelineType)
{
...

	{
		uint32_t descSetLimit =
			glm::min(VKCONST_MAXDESCSET_PERLIST, bundle->m_gpu->vk_propsDev.properties.limits.maxBoundDescriptorSets);
...
void vk_cmdBindPipeline(struct tgfx_commandBundle* bndl, unsigned long long sortKey, struct tgfx_pipeline* pipeline)
{
	CMDBUNDLE_VKOBJ* bundle = getOBJ<CMDBUNDLE_VKOBJ>(bndl);
...
void vk_cmdSetViewport(struct tgfx_commandBundle* bndl, unsigned long long sortKey, const tgfx_viewportInfo* viewport)
{
	CMDBUNDLE_VKOBJ* bundle = getOBJ<CMDBUNDLE_VKOBJ>(bndl);
...
void vk_cmdSetScissor(struct tgfx_commandBundle* bndl,
					  unsigned long long sortKey,
					  const tgfx_ivec2* offset,
					  const tgfx_uvec2* size)
{
	CMDBUNDLE_VKOBJ* bundle = getOBJ<CMDBUNDLE_VKOBJ>(bndl);
...
void vk_cmdSetDepthBounds(struct tgfx_commandBundle* bndl, unsigned long long sortKey, float min, float max)
{
	CMDBUNDLE_VKOBJ* bundle = getOBJ<CMDBUNDLE_VKOBJ>(bndl);
...
void vk_cmdBindVertexBuffers(struct tgfx_commandBundle* bndl,
							 unsigned long long sortKey,
...
void vk_cmdBindIndexBuffer(struct tgfx_commandBundle* bndl,
						   unsigned long long sortKey,
...
void vk_cmdDrawNonIndexedDirect(struct tgfx_commandBundle* bndl,
								unsigned long long sortKey,
...
void vk_cmdDrawIndexedDirect(struct tgfx_commandBundle* bndl,
							 unsigned long long sortKey,
...
void vk_cmdExecuteIndirect(struct tgfx_commandBundle* bndl,
						   unsigned long long sortKey,
						   ...
						   struct tgfx_extension* const* exts)
{
	...
	uint32_t allocSize = sizeof(vkCmdStruct_executeIndirect::vk_indirectOperationState) * cmd->opStateCount;
	cmd->opStates = (vkCmdStruct_executeIndirect::vk_indirectOperationState*)VK_MEMOFFSET_TO_POINTER(
...
void vk_cmdBarrierTexture(struct tgfx_commandBundle* bndl,
						  unsigned long long key,
						  struct tgfx_texture* i_texture,
						  image_access_tgfx lastAccess,
						  image_access_tgfx nextAccess,
...
void vk_cmdDispatch(struct tgfx_commandBundle* bndl, unsigned long long key, const tgfx_uvec3* dispatchSize)
{
	CMDBUNDLE_VKOBJ* bundle = getOBJ<CMDBUNDLE_VKOBJ>(bndl);
...
void vk_cmdCopyBufferToTexture(struct tgfx_commandBundle* bndl,
							   unsigned long long key,
							   ...
							   struct tgfx_extension* const* exts)
{
	...
	{
		cmd->vk_copy.imageSubresource.aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT;
	}
	else
	{
...
void vk_cmdCopyBufferToBuffer(struct tgfx_commandBundle* bndl,
							  unsigned long long key,
...
void vk_cmdPushConstant(
	struct tgfx_commandBundle* bndl, unsigned long long key, unsigned char offset, unsigned char size, const void* d)
{
	CMDBUNDLE_VKOBJ* bundle = getOBJ<CMDBUNDLE_VKOBJ>(bndl);
	...
	for (uint32_t bundleListIndx = 0; bundleListIndx < cmdBundleCount; bundleListIndx++)
	{
		...
		if (vkCmdBuffer == VK_NULL_HANDLE)
		{
			vk_allocateCmdBuffer(getQueueFam(bundle->m_gpu, queueFamIndx),
								 ...
								 bundle->vk_cmdPools[queueFamIndx],
								 &vkCmdBuffer,
								 1);
