...
// For example: renderer.cpp will add passes and calls, rg_***.cpp will reconstruct RG or record CBs
// etc So don't include this in contentmanager.cpp or core.cpp etc.
#include <stdint.h>
...
struct RenderGraph {
  ...
  struct MainPassCOMMON {
    ...
    VK_PASS_ID_TYPE ALLSUBPASSESCOUNT =
      VK_PASS_INVALID_ID; // This is all subpasses that can be waited upon
    // This is to identify quickly which passes are waited as what in whole mainpass
    // So this is not precise, just helps if algorithm should further examine (by searching
    // subpasses, last frame dependencies etc.)
    struct MainPassWait {
    ...
    MainPassCOMMON(VK_PASSTYPE type, uint16_t passcount = VK_PASS_INVALID_ID)
      : PASSTYPE(type), ALLSUBPASSESCOUNT(passcount) {}
  };
  ...
  struct SubDP_VK {
    ...
    uint16_t VKRP_BINDINDEX, RG_BINDINDEX; // RG bindindex is the id in all subdps of the same dp,
                                           // vkrp_bindindex is the id in VkRenderpass object
  ...
  // VkRenderPass related: Subpass defines a valid sync point for Tile-Based renderer drivers in
  // Vulkan 	so they need to define how they access to rtslotset with irtslotset BarrierOnly related:
  // Subpass doesn't define the thing above but subpass only waits for some operations of other
  // passes
  struct DP_VK {
  ...
  struct dispatchcall_vk {
    ...
    dispatchcall_vk() {
      for (uint32_t i = 0; i < VKCONST_MAXDESCSET_PERLIST * 2; i++) {
        BINDINGTABLEIDs[i] = UINT16_MAX;
      }
    }
  };
  ...
  static uint32_t               GetSize_ofType(VK_PASSTYPE type, void* pass) {
                  switch (type) {
                    case VK_PASSTYPE::DP: {
                      DP_VK* dp = ( DP_VK* )pass;
                      return sizeof(DP_VK) + (dp->base.ALLSUBPASSESCOUNT * sizeof(SubDP_VK));
      } break;
                    case VK_PASSTYPE::CP: {
                      CP_VK* cp = ( CP_VK* )pass;
                      return sizeof(CP_VK) + (cp->base.ALLSUBPASSESCOUNT * sizeof(SubCP_VK));
      } break;
                    case VK_PASSTYPE::TP: {
                      TP_VK* tp = ( TP_VK* )pass;
                      return sizeof(TP_VK) + tp->SubPassesList_SizeInBytes;
      } break;
                    case VK_PASSTYPE::WP: {
                      WP_VK* wp = ( WP_VK* )pass;
                      return sizeof(WP_VK);
        ...
        printer(result_tgfx_FAIL,
                              "Pass is not supported by the GetSize_ofType(), there is a possible memory bug!");
        return UINT32_MAX;
    }
  }
  void* GetNextPass(void* currentPass, VK_PASSTYPE type) {
    ...
    if (current < PASSES_PTR ||
        current > PASSES_PTR + (VKCONST_VIRMEM_RGALLOCATION_PAGECOUNT * VKCONST_VIRMEMPAGESIZE)) {
      printer(result_tgfx_FAIL, "Passes current ptr is wrong, please report this!");
    }
#endif }
    while (current <
           PASSES_PTR + (VKCONST_VIRMEM_RGALLOCATION_PAGECOUNT * VKCONST_VIRMEMPAGESIZE)) {
  ...
  // This stores reconstruction status, even though old valid RG isn't changed after new one is
