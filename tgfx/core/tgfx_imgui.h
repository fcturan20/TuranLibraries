...
typedef struct tgfx_dearimgui {
	...
	unsigned char (*Create_Window)
		(const char* title, unsigned char* should_close, unsigned char has_menubar);
	void (*End_Window)();
	...
	unsigned char (*SelectList_OneLine)
		(const char* name, unsigned int* selected_index, const char* const* names);
	void (*Selectable)(const char* name, unsigned char* is_selected);
	// Create a box of selectable items in one-line of code!
	// @return 1: Any item is selected.
	//		Selected item's index is the selected_index's pointer's value!
	unsigned char (*Selectable_ListBox)
		(const char* name, int* selected_index, const char* const* item_names);
	void (*Display_Texture)
	(
		texture_tgfxhnd hnd, unsigned int width, unsigned int height,
	...
	unsigned char (*Slider_Float)
		(const char* name, float* data, float min, float max);
	unsigned char (*Slider_Vec2)
		(const char* name, vec2_tgfx* data, float min, float max);
	unsigned char (*Slider_Vec3)
		(const char* name, vec3_tgfx* data, float min, float max);
	unsigned char (*Slider_Vec4)
		(const char* name, vec4_tgfx* data, float min, float max);
} dearimgui_tgfx;
