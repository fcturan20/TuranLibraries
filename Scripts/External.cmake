...
function(tcmake_run_regen_script)
    ...
    if (result EQUAL 0)
        ...
        message(FATAL_ERROR "Failed to run regen.py.\nOutput:\n${output}\nError:\n${error_output}")
    endif()
endfunction()
...
function(tcmake_add_vcpkg_dependency target_name dependency target_dep_name)
    ...
    if (NOT TARGET ${target_dep_name} AND (NOT DEFINED ${target_dep_name} OR NOT ${target_dep_name}))
        # Some ports expose Find<Package>.cmake variables (e.g. Stb_INCLUDE_DIR)
        # instead of imported config targets.
    ...
    elseif (DEFINED ${target_dep_name} AND ${target_dep_name})
        set(_resolved_include_dirs "${${target_dep_name}}")
        message(STATUS "Dependency ${dependency} found with include dirs from variable ${target_dep_name}: ${_resolved_include_dirs}")
        target_include_directories(${target_name} PRIVATE ${_resolved_include_dirs})
    else()
