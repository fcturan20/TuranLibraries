...
void Start_RenderGraphConstruction()
{
	...
	RG_RECDATA::RG_wip =
		(RenderGraph*)VK_ALLOCATE_AND_GETPTR(RG_RECDATA::VKGLOBAL_VIRMEM_RGRECDYNAMICMEM, sizeof(RenderGraph));
...
static void Fill_SubpassStructs(IRTSLOTSET_VKOBJ* slotset,
								subdrawpassaccess_tgfx WaitedOp,
								subdrawpassaccess_tgfx ContinueOp,
								unsigned char SubpassIndex,
								VkSubpassDescription& descs,
								VkSubpassDependency& dependencies)
{
	...
	if (BASESLOTSET->PERFRAME_SLOTSETs[0].DEPTHSTENCIL_SLOT)
	{
		...
		DS_Attach =
			(VkAttachmentReference*)VK_ALLOCATE_AND_GETPTR(VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(VkAttachmentReference));
		rendergpu->EXTMANAGER()->Fill_DepthAttachmentReference(*DS_Attach,
															   BASESLOTSET->PERFRAME_SLOTSETs[0].COLORSLOTs_COUNT,
															   depthslot->RT->CHANNELs,
															   slotset->DEPTH_OPTYPE,
															   slotset->STENCIL_OPTYPE);
	}
								   ...
								   drawpass_tgfx_handle* DPHandle)
{
	...
	for (unsigned int i = 0; i < SubDPsLISTSIZE; i++)
	{
		...
		Fill_SubpassStructs(Subpass_Slotset,
							Subpass_gfxdesc->WAITOP,
							Subpass_gfxdesc->CONTINUEOP,
							i,
							SubpassDescs[i],
							SubpassDepends[i]);
	}
	...
	// vkRenderPass generation
	{
		...
		if (SLOTSET->PERFRAME_SLOTSETs[0].DEPTHSTENCIL_SLOT)
		{
			rendergpu->EXTMANAGER()->Fill_DepthAttachmentDescription(AttachmentDescs[attachmentdescs_count++],
																	 SLOTSET->PERFRAME_SLOTSETs[0].DEPTHSTENCIL_SLOT);
		}
		...
		if (vkCreateRenderPass(
				rendergpu->LOGICALDEVICE(), &RenderPass_CreationInfo, nullptr, &VKDrawPass.RenderPassObject) !=
			VK_SUCCESS)
		{
	...
	RenderGraph::DP_VK* DP_final =
		(RenderGraph::DP_VK*)VK_ALLOCATE_AND_GETPTR(RG_RECDATA::VKGLOBAL_VIRMEM_RGRECDYNAMICMEM, dp_malloc_size);
	...
	for (unsigned int i = 0, s = 0, rp_bi = 0; i < DP_final->base.ALLSUBPASSESCOUNT; i++)
	{
		...
		while (!desc)
		{
			desc = RenderGraph::getDesc_fromHandle<RG_RECDATA::SubDPDESC>(*(VKDATAHANDLE*)&subDPdescs[s++]);
		}
									   ...
									   transferpass_tgfx_handle* TPHandle)
{
	...
	for (unsigned int subpass_i = 0; subpass_i < subtplistsize; subpass_i++)
	{
		...
		RG_RECDATA::SubTPDESC* desc =
			RenderGraph::getDesc_fromHandle<RG_RECDATA::SubTPDESC>(*(VKDATAHANDLE*)&subTPdescs[subpass_i]);
									  ...
									  computepass_tgfx_handle* CPHandle)
{
	...
	for (unsigned int subpass_i = 0; subpass_i < subcplistsize; subpass_i++)
	{
		...
		RG_RECDATA::SubCPDESC* desc =
			RenderGraph::getDesc_fromHandle<RG_RECDATA::SubCPDESC>(*(VKDATAHANDLE*)&subCPdescs[subpass_i]);
#ifdef VULKAN_DEBUGGING
									 ...
									 windowpass_tgfx_handle* WindowPassHandle)
{
	...
	RenderGraph::WP_VK* WP = (RenderGraph::WP_VK*)VK_ALLOCATE_AND_GETPTR(RG_RECDATA::VKGLOBAL_VIRMEM_RGRECDYNAMICMEM,
																		 sizeof(RenderGraph::WP_VK));
...
inline static bool Check_WaitHandles()
{
	...
	while (current_PASS_ID < RG_RECDATA::ID_GENERATOR.load() &&
		   last_ptr < uintptr_t(VKCONST_VIRMEMSPACE_BEGIN) + RG_RECDATA::RG_wip->PASSES_PTR +
...
unsigned char Finish_RenderGraphConstruction(subdrawpass_tgfx_handle IMGUI_Subpass)
{
	...
	if (!Check_WaitHandles())
	{
		...
		RenderGraph::setRGSTATUS(RGReconstructionStatus::Invalid); // User can change only waits of a pass, so all
																   // rendergraph falls to invalid in this case
	...
	if (imgui)
	{
		...
		if (IMGUI_Subpass)
		{
			...
			if (imgui->Get_IMGUIStatus() == imgui_vk::IMGUI_STATUS::UNINITIALIZED)
			{
				...
				if (vkCreateDescriptorPool(rendergpu->LOGICALDEVICE(), &descpool_ci, nullptr, &imgui->descpool) !=
					VK_SUCCESS)
				{
...
static passwaitdescription_tgfx_handle CreatePassWait_SubDrawPass(drawpass_tgfx_handle* PassHandle,
																  unsigned int SubDPIndex,
																  waitsignaldescription_tgfx_handle* WaitInfos,
																  unsigned char isLastFrame)
{
	if (!PassHandle)
	{
		printer(result_tgfx_INVALIDARGUMENT, "CreatePassWait_SubDrawPass()'s first argument can't be nullptr!");
		return nullptr;
	}
...
static passwaitdescription_tgfx_handle CreatePassWait_SubComputePass(computepass_tgfx_handle* PassHandle,
																	 unsigned int SubCPIndex,
																	 waitsignaldescription_tgfx_handle WaitInfo,
																	 unsigned char isLastFrame)
{
	if (!PassHandle)
	{
		printer(result_tgfx_INVALIDARGUMENT, "CreatePassWait_SubComputePass()'s first argument can't be nullptr!");
		return nullptr;
	}
...
static passwaitdescription_tgfx_handle CreatePassWait_SubTransferPass(transferpass_tgfx_handle* PassHandle,
																	  unsigned int SubTPIndex,
																	  waitsignaldescription_tgfx_handle WaitInfo,
																	  unsigned char isLastFrame)
{
	if (!PassHandle)
	{
		printer(result_tgfx_INVALIDARGUMENT, "CreatePassWait_SubTransferPass()'s first argument can't be nullptr!");
		return nullptr;
	}
...
static subdrawpassdescription_tgfx_handle CreateSubDrawPassDescription(passwaitdescription_tgfx_listhandle waitdescs,
																	   inheritedrtslotset_tgfx_handle irtslotset,
																	   subdrawpassaccess_tgfx WaitOP,
																	   subdrawpassaccess_tgfx ContinueOP)
{
	RG_RECDATA::SubDPDESC* desc =
		(RG_RECDATA::SubDPDESC*)VK_ALLOCATE_AND_GETPTR(VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(RG_RECDATA::SubDPDESC));
	if (irtslotset)
	{
...
{
	RG_RECDATA::SubCPDESC* desc =
		(RG_RECDATA::SubCPDESC*)VK_ALLOCATE_AND_GETPTR(VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(RG_RECDATA::SubCPDESC));
	desc->waits = waitdescs;
...
#endif
	RG_RECDATA::SubTPDESC* desc =
		(RG_RECDATA::SubTPDESC*)VK_ALLOCATE_AND_GETPTR(VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(RG_RECDATA::SubTPDESC));
	desc->waits = waits;
	desc->type = type_i;
...
waitsignaldescription_tgfx_handle waitsignal_cs(unsigned char fds, unsigned char sdf, unsigned char lsdfv)
{
	return nullptr;
}
waitsignaldescription_tgfx_handle waitsignal_drawindirectconsume()
{
	return nullptr;
}
