...
{
	return priv->findSystem(name);
}

//                        COMPONENT FUNCTIONS
////////////////////////////////////////////////////////////////////////////

struct tlComponentTypeID* addComponentType(const char* name,
										   void* mainType,
...
struct tlEntityType* addEntityType(const struct tlComponentTypeID* const* compTypeList, unsigned int listSize)
{
	ecs_entityType type;
	type.compCount = listSize;
	type.typeID = f_vector->size(priv->v_entityTypes);
	type.v_entityList = f_vector->create(mainMemBlock, listSize * sizeof(tlComponent*), 0, 1 << 20, 0);
	type.compTypeHndlesList =
		(tlComponentTypeID**)standard_alloc->malloc(mainMemBlock, sizeof(tlComponentTypeID) * listSize);
...
tlEntity* createEntity(struct tlEntityType* typeHandle)
{
	...
	tlComponent** compHndsList = reinterpret_cast<tlComponent**>(reinterpret_cast<uintptr_t>(eType->v_entityList) +
...
unsigned char doesContains_entityType(struct tlEntityType* eTypeHnd, struct tlComponentTypeID* compType)
{
...
TCResult TCECS_Initialize(const void** outECSAPI)
{
	...
	*outECSAPI = TCECS;
	...
	ALLOCATOR_TAPI_PLUGIN_LOAD_TYPE allocSys = (ALLOCATOR_TAPI_PLUGIN_LOAD_TYPE)initializeAllocatorPtr();
	...
	TCECSServices* ecs = (TCECSServices*)allocSys->endOfPage->malloc(mainMemBlock, sizeof(TCECSServices));
