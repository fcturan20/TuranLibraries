...
vkext_timelineSemaphore::vkext_timelineSemaphore(GPU_VKOBJ* gpu) : vkext_interface(gpu, &props, &features)
{
...
TCResult GetFenceValue(TGfxFence i_fence, unsigned long long* value)
{
	...
	if (vkGetSemaphoreCounterValueKHR_loaded(gpu->vk_logical, fence->vk_timelineSemaphore, value) != VK_SUCCESS)
	{
...
void vkext_timelineSemaphore::inspect()
{
	...
	m_gpu->ext()->m_activeDevExtNames[m_gpu->ext()->m_devExtCount++] = (VK_KHR_TIMELINE_SEMAPHORE_EXTENSION_NAME);
