...
struct RG_RECDATA {
  ...
  enum class WaitDescTYPE : unsigned char { subDP = 0, subCP = 1, subTP = 2 };
  struct SubDPDESC {
    ...
    static SubDPDESC* getDesc_fromHandle(VKDATAHANDLE handle) {
      if (handle.type != VKHANDLETYPEs::RGRECDATA_SUBDPDESC) {
        return nullptr;
      }
  ...
  struct SubTPDESC {
    ...
    static SubTPDESC* getDesc_fromHandle(VKDATAHANDLE handle) {
      if (handle.type != VKHANDLETYPEs::RGRECDATA_SUBTPDESC) {
        return nullptr;
      }
  ...
  struct SubCPDESC {
    ...
    static RenderGraph::WaitDesc_SubCP* getDesc_fromHandle(VKDATAHANDLE handle) {
      if (handle.type != VKHANDLETYPEs::RGRECDATA_SUBCPDESC) {
        return nullptr;
      }
  ...
  template <typename T>
  static VKDATAHANDLE getHandle_ofDesc(T* obj) {
    return obj->getHandle_ofDesc();
  }
...
RGReconstructionStatus renderer_public::RGSTATUS() {
  if (!VKGLOBAL_RG) {
    if (!RG_RECDATA::RG_wip) {
      return RGReconstructionStatus::Invalid;
    } else {
      return RG_RECDATA::RG_wip->STATUS;
    }
  }
...
RenderGraph::DP_VK* RenderGraph::SubDP_VK::getDP() {
  ...
  if (dp->base.PASS_ID != DP_ID) {
    printer(result_tgfx_FAIL, "SubDP-MainDP doesn't match!");
  }
  return dp;
}
...
void Start_RenderGraphConstruction() {
  ...
  if (renderer->RGSTATUS() == RGReconstructionStatus::FinishConstructionCalled ||
      renderer->RGSTATUS() == RGReconstructionStatus::StartedConstruction) {
    printer(result_tgfx_FAIL,
            "GFXRENDERER->Start_RenderGraphCreation() has failed because you have wrong function "
            "call order!");
    return;
  }

  RG_RECDATA::VKGLOBAL_VIRMEM_RGRECDYNAMICMEM =
    vk_virmem::allocate_dynamicmem(RenderGraph::VKCONST_VIRMEM_RGALLOCATION_PAGECOUNT);
  RG_RECDATA::RG_wip = ( RenderGraph* )VK_ALLOCATE_AND_GETPTR(
    RG_RECDATA::VKGLOBAL_VIRMEM_RGRECDYNAMICMEM, sizeof(RenderGraph));
  RG_RECDATA::RG_wip->PASSES_PTR =
    VK_POINTER_TO_MEMOFFSET(RG_RECDATA::RG_wip) + sizeof(RenderGraph);

  RenderGraph::setRGSTATUS(RGReconstructionStatus::StartedConstruction);
}

static void Fill_ColorVkAttachmentDescription(VkAttachmentDescription& Desc,
                                              const colorslot_vk*      Attachment) {
  Desc               = {};
...
static void Fill_SubpassStructs(IRTSLOTSET_VKOBJ* slotset, subdrawpassaccess_tgfx WaitedOp,
                                subdrawpassaccess_tgfx ContinueOp, unsigned char SubpassIndex,
                                VkSubpassDescription& descs, VkSubpassDependency& dependencies) {
  RTSLOTSET_VKOBJ* BASESLOTSET =
    contentmanager->GETRTSLOTSET_ARRAY().getOBJbyINDEX(slotset->BASESLOTSET);
  VkAttachmentReference* COLOR_ATTACHMENTs = ( VkAttachmentReference* )VK_ALLOCATE_AND_GETPTR(
    VKGLOBAL_VIRMEM_CURRENTFRAME,
    sizeof(VkAttachmentReference) * BASESLOTSET->PERFRAME_SLOTSETs[0].COLORSLOTs_COUNT);
  ...
  if (BASESLOTSET->PERFRAME_SLOTSETs[0].DEPTHSTENCIL_SLOT) {
    ...
    DS_Attach = ( VkAttachmentReference* )VK_ALLOCATE_AND_GETPTR(VKGLOBAL_VIRMEM_CURRENTFRAME,
                                                                 sizeof(VkAttachmentReference));
    rendergpu->EXTMANAGER()->Fill_DepthAttachmentReference(
      *DS_Attach, BASESLOTSET->PERFRAME_SLOTSETs[0].COLORSLOTs_COUNT, depthslot->RT->CHANNELs,
      slotset->DEPTH_OPTYPE, slotset->STENCIL_OPTYPE);
  }
  ...
  if (SubpassIndex) {
    ...
    Find_SubpassAccessPattern(WaitedOp, true, dependencies.srcStageMask,
                              dependencies.srcAccessMask);
    Find_SubpassAccessPattern(ContinueOp, true, dependencies.dstStageMask,
...
result_tgfx check_waitdescs_of_subpass(passwaitdescription_tgfx_listhandle waits,
                                       drawpass_tgfx_handle**              dphandle,
  ...
  for (unsigned int wait_i = 0; wait_i < waitlistsize; wait_i++) {
    ...
    switch (handle.type) {
      case VKHANDLETYPEs::WAITDESC_SUBDP: {
        RenderGraph::WaitDesc_SubDP* waitdesc =
          RenderGraph::getDesc_fromHandle<RenderGraph::WaitDesc_SubDP>(handle);
        if (*dphandle) {
          if (waitdesc->passhandle != *dphandle) {
            return printer(1);
          }
        } else {
          *dphandle = waitdesc->passhandle;
        }
      } break;
      case VKHANDLETYPEs::WAITDESC_SUBCP: {
        RenderGraph::WaitDesc_SubCP* waitdesc =
          RenderGraph::getDesc_fromHandle<RenderGraph::WaitDesc_SubCP>(handle);
        if (*cphandle) {
          if (waitdesc->passhandle != *cphandle) {
            return printer(1);
          }
        } else {
          *cphandle = waitdesc->passhandle;
        }
      } break;
      case VKHANDLETYPEs::WAITDESC_SUBTP: {
        RenderGraph::WaitDesc_SubTP* waitdesc =
          RenderGraph::getDesc_fromHandle<RenderGraph::WaitDesc_SubTP>(handle);
        if (*tphandle) {
          if (waitdesc->passhandle != *tphandle) {
            return printer(1);
          }
        } else {
          *tphandle = waitdesc->passhandle;
        }
      } break;
      default: return printer(2);
    }
  }
  return result_tgfx_SUCCESS;
}
static result_tgfx Create_DrawPass(subdrawpassdescription_tgfx_listhandle subDPdescs,
                                   rtslotset_tgfx_handle                  RTSLOTSET_ID,
                                   subdrawpass_tgfx_listhandle            SubDrawPassHandles,
                                   drawpass_tgfx_handle*                  DPHandle) {
  if (renderer->RGSTATUS() != RGReconstructionStatus::StartedConstruction) {
    printer(result_tgfx_FAIL,
            "GFXRENDERER->CreateDrawPass() has failed because you first need to call "
            "Start_RenderGraphCreation()!");
    return result_tgfx_FAIL;
  }
  RTSLOTSET_VKOBJ* SLOTSET =
    contentmanager->GETRTSLOTSET_ARRAY().getOBJfromHANDLE(*( VKOBJHANDLE* )&RTSLOTSET_ID);
  for (unsigned int i = 0; i < SLOTSET->PERFRAME_SLOTSETs[0].COLORSLOTs_COUNT; i++) {
    ...
    if (slot.RT_OPERATIONTYPE == operationtype_tgfx_UNUSED) {
      printer(result_tgfx_INVALIDARGUMENT,
              "Create_DrawPass() has failed because you can't give an unused Color RT Slot to the "
              "Draw Pass! You either don't give it, or use it");
      return result_tgfx_INVALIDARGUMENT;
    }
  }
  if (SLOTSET->PERFRAME_SLOTSETs[0].DEPTHSTENCIL_SLOT) {
    if (SLOTSET->PERFRAME_SLOTSETs[0].DEPTHSTENCIL_SLOT->DEPTH_OPTYPE ==
        operationtype_tgfx_UNUSED) {
      printer(result_tgfx_FAIL,
              "Create_DrawPass() has failed because you can't give an unused Depth RT Slot to the "
              "Draw Pass! You either don't give it, or use it");
      return result_tgfx_INVALIDARGUMENT;
    }
  }
  ...
  {
    RG_RECDATA::SubDPDESC* dpdesc =
      RenderGraph::getDesc_fromHandle<RG_RECDATA::SubDPDESC>(*( VKDATAHANDLE* )&subDPdescs[0]);
    if (dpdesc->IRTSLOTSET_ID == UINT32_MAX) {
      printer(result_tgfx_INVALIDARGUMENT, "First subdrawpass should have valid irtslotset!");
      return result_tgfx_INVALIDARGUMENT;
    }
    IRTSLOTSET_VKOBJ* ISET =
      contentmanager->GETIRTSLOTSET_ARRAY().getOBJbyINDEX(dpdesc->IRTSLOTSET_ID);
    if (!ISET) {
      printer(
        result_tgfx_FAIL,
        "GFXRenderer->Create_DrawPass() has failed because first inherited slotset isn't valid!");
      return result_tgfx_INVALIDARGUMENT;
    }
    if (contentmanager->GETRTSLOTSET_ARRAY().getOBJbyINDEX(ISET->BASESLOTSET) != SLOTSET) {
      printer(result_tgfx_FAIL,
              "GFXRenderer->Create_DrawPass() has failed because first inherited slotset isn't "
              "inherited from the DrawPass' Base Slotset!");
      return result_tgfx_INVALIDARGUMENT;
    }
    TGFXLISTCOUNT(core_tgfx_main, dpdesc->waits, waitlist_size);
    for (unsigned int wait_i = 0; wait_i < waitlist_size; wait_i++) {
      if (!dpdesc->waits[wait_i]) {
        continue;
      }
      ...
      if (handle.type == VKHANDLETYPEs::WAITDESC_SUBDP) {
        ...
        if (waitdesc->passhandle == DPHandle) {
          printer(result_tgfx_INVALIDARGUMENT,
                  "First subdrawpass can't wait for any subpass of the same drawpass");
          return result_tgfx_INVALIDARGUMENT;
        }
      }
    }
  ...
  for (unsigned int subdp_i = 1; subdp_i < SubDPsLISTSIZE; subdp_i++) {
    RG_RECDATA::SubDPDESC* dpdesc =
      RenderGraph::getDesc_fromHandle<RG_RECDATA::SubDPDESC>(*( VKDATAHANDLE* )&subDPdescs[0]);
    if (dpdesc->IRTSLOTSET_ID == UINT32_MAX) {
      BarrierOnly_subDPs_COUNT++;
    } else {
      RenderPass_subDPs_COUNT++;
    }
#ifdef VULKAN_DEBUGGING
    IRTSLOTSET_VKOBJ* ISET =
      contentmanager->GETIRTSLOTSET_ARRAY().getOBJbyINDEX(dpdesc->IRTSLOTSET_ID);
  ...
  VkSubpassDependency* SubpassDepends = ( VkSubpassDependency* )VK_ALLOCATE_AND_GETPTR(
    VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(VkSubpassDependency) * RenderPass_subDPs_COUNT);
  VkSubpassDescription* SubpassDescs = ( VkSubpassDescription* )VK_ALLOCATE_AND_GETPTR(
    VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(VkSubpassDescription) * RenderPass_subDPs_COUNT);

  for (unsigned int i = 0; i < SubDPsLISTSIZE; i++) {
    RG_RECDATA::SubDPDESC* Subpass_gfxdesc =
      RenderGraph::getDesc_fromHandle<RG_RECDATA::SubDPDESC>(*( VKDATAHANDLE* )&subDPdescs[i]);
    if (!Subpass_gfxdesc) {
      continue;
    }
    IRTSLOTSET_VKOBJ* Subpass_Slotset =
      contentmanager->GETIRTSLOTSET_ARRAY().getOBJbyINDEX(Subpass_gfxdesc->IRTSLOTSET_ID);
    Fill_SubpassStructs(Subpass_Slotset, Subpass_gfxdesc->WAITOP, Subpass_gfxdesc->CONTINUEOP, i,
                        SubpassDescs[i], SubpassDepends[i]);
  }

  // vkRenderPass generation
  {
    VkAttachmentDescription* AttachmentDescs = ( VkAttachmentDescription* )VK_ALLOCATE_AND_GETPTR(
      VKGLOBAL_VIRMEM_CURRENTFRAME,
      sizeof(VkAttachmentDescription) * (SLOTSET->PERFRAME_SLOTSETs[0].COLORSLOTs_COUNT + 1));
    uint8_t attachmentdescs_count = 0;
    // Create Attachment Descriptions
    for (unsigned int i = 0; i < SLOTSET->PERFRAME_SLOTSETs[0].COLORSLOTs_COUNT; i++) {
      Fill_ColorVkAttachmentDescription(AttachmentDescs[attachmentdescs_count++],
                                        &SLOTSET->PERFRAME_SLOTSETs[0].COLOR_SLOTs[i]);
    }
    if (SLOTSET->PERFRAME_SLOTSETs[0].DEPTHSTENCIL_SLOT) {
      rendergpu->EXTMANAGER()->Fill_DepthAttachmentDescription(
        AttachmentDescs[attachmentdescs_count++], SLOTSET->PERFRAME_SLOTSETs[0].DEPTHSTENCIL_SLOT);
    }
    ...
    if (vkCreateRenderPass(rendergpu->LOGICALDEVICE(), &RenderPass_CreationInfo, nullptr,
                           &VKDrawPass.RenderPassObject) != VK_SUCCESS) {
  ...
  uint32_t dp_malloc_size =
    sizeof(RenderGraph::DP_VK) + (SubDPsLISTSIZE * sizeof(RenderGraph::SubDP_VK));
  RenderGraph::DP_VK* DP_final = ( RenderGraph::DP_VK* )VK_ALLOCATE_AND_GETPTR(
    RG_RECDATA::VKGLOBAL_VIRMEM_RGRECDYNAMICMEM, dp_malloc_size);
  ...
  RenderGraph::SubDP_VK* Final_Subpasses =
    ( RenderGraph::SubDP_VK* )(uintptr_t(DP_final) + sizeof(RenderGraph::DP_VK));
  for (unsigned int i = 0, s = 0, rp_bi = 0; i < DP_final->base.ALLSUBPASSESCOUNT; i++) {
    // Get next valid desc
    RG_RECDATA::SubDPDESC* desc =
      RenderGraph::getDesc_fromHandle<RG_RECDATA::SubDPDESC>(*( VKDATAHANDLE* )&subDPdescs[s++]);
    while (!desc) {
      desc =
        RenderGraph::getDesc_fromHandle<RG_RECDATA::SubDPDESC>(*( VKDATAHANDLE* )&subDPdescs[s++]);
    }
...
static result_tgfx Create_TransferPass(subtransferpassdescription_tgfx_listhandle subTPdescs,
                                       subtransferpass_tgfx_listhandle            subTPHandles,
                                       transferpass_tgfx_handle*                  TPHandle) {
  if (renderer->RGSTATUS() != RGReconstructionStatus::StartedConstruction) {
    printer(
      result_tgfx_FAIL,
      "You should call Start_RenderGraphConstruction() before RENDERER->Create_TransferPass()!");
    return result_tgfx_FAIL;
  }
  ...
  for (unsigned int subpass_i = 0; subpass_i < subtplistsize; subpass_i++) {
    if (!subTPdescs[subpass_i]) {
      continue;
    }
    RG_RECDATA::SubTPDESC* desc = RenderGraph::getDesc_fromHandle<RG_RECDATA::SubTPDESC>(
      *( VKDATAHANDLE* )&subTPdescs[subpass_i]);
    if (desc->type == transferpasstype_tgfx_BARRIER) {
      barrier_subtpcount++;
    } else if (desc->type == transferpasstype_tgfx_COPY) {
      copy_subtpcount++;
    }
#ifdef VULKAN_DEBUGGING
    else {
  ...
  uint32_t size_of_subtps = (sizeof(RenderGraph::SubTPCOPY_VK) * copy_subtpcount) +
                            (sizeof(RenderGraph::SubTPBARRIER_VK) * barrier_subtpcount);
  uint32_t sizeof_malloc = sizeof(RenderGraph::TP_VK) + size_of_subtps;
  uint32_t tp_memoffset =
    vk_virmem::allocate_from_dynamicmem(RG_RECDATA::VKGLOBAL_VIRMEM_RGRECDYNAMICMEM, sizeof_malloc);
  RenderGraph::TP_VK* tp =
    ( RenderGraph::TP_VK* )(uintptr_t(VKCONST_VIRMEMSPACE_BEGIN) + tp_memoffset);
...
static result_tgfx Create_ComputePass(subcomputepassdescription_tgfx_listhandle subCPdescs,
                                      subcomputepass_tgfx_listhandle            subCPHandles,
                                      computepass_tgfx_handle*                  CPHandle) {
  if (renderer->RGSTATUS() != RGReconstructionStatus::StartedConstruction) {
    printer(
      result_tgfx_FAIL,
      "You should call Start_RenderGraphConstruction() before RENDERER->Create_ComputePass()!");
    return result_tgfx_FAIL;
  }
  ...
  for (unsigned int subpass_i = 0; subpass_i < subcplistsize; subpass_i++) {
    if (!subCPdescs[subpass_i]) {
      continue;
    }
    RG_RECDATA::SubCPDESC* desc = RenderGraph::getDesc_fromHandle<RG_RECDATA::SubCPDESC>(
      *( VKDATAHANDLE* )&subCPdescs[subpass_i]);
#ifdef VULKAN_DEBUGGING
    check_waitdescs_of_subpass(desc->waits, &merged_dp, &CPHandle, &merged_tp);
#endif
    subcpcount++;
  }
  uint32_t sizeof_malloc =
    (sizeof(RenderGraph::SubCP_VK) * subcpcount) + sizeof(RenderGraph::CP_VK);
  uint32_t cp_memoffset =
    vk_virmem::allocate_from_dynamicmem(RG_RECDATA::VKGLOBAL_VIRMEM_RGRECDYNAMICMEM, sizeof_malloc);
  RenderGraph::CP_VK* CP =
    ( RenderGraph::CP_VK* )(uintptr_t(VKCONST_VIRMEMSPACE_BEGIN) + cp_memoffset);
  *CP                        = RenderGraph::CP_VK();
  CP->base.PASS_ID           = RG_RECDATA::ID_GENERATOR.fetch_add(1);
  CP->base.ALLSUBPASSESCOUNT = subcpcount;
  RenderGraph::SubCP_VK* final_subcps =
    ( RenderGraph::SubCP_VK* )(uintptr_t(CP) + sizeof(RenderGraph::CP_VK));
...
static result_tgfx Create_WindowPass(passwaitdescription_tgfx_listhandle WaitDescriptions,
                                     windowpass_tgfx_handle*             WindowPassHandle) {
  if (renderer->RGSTATUS() != RGReconstructionStatus::StartedConstruction) {
    printer(
      result_tgfx_FAIL,
      "You should call Start_RenderGraphConstruction() before RENDERER->Create_ComputePass()!");
  ...
  RenderGraph::WP_VK* WP = ( RenderGraph::WP_VK* )VK_ALLOCATE_AND_GETPTR(
    RG_RECDATA::VKGLOBAL_VIRMEM_RGRECDYNAMICMEM, sizeof(RenderGraph::WP_VK));
...
inline static bool Check_WaitHandles() {
  ...
  while (current_PASS_ID < RG_RECDATA::ID_GENERATOR.load() &&
         last_ptr <
           uintptr_t(VKCONST_VIRMEMSPACE_BEGIN) + RG_RECDATA::RG_wip->PASSES_PTR +
             (RG_RECDATA::RG_wip->VKCONST_VIRMEM_RGALLOCATION_PAGECOUNT * VKCONST_VIRMEMPAGESIZE)) {
    ...
    switch (type) {
      case VK_PASSTYPE::DP: {
        ...
        if (current_PASS_ID != dp->base.PASS_ID) {
          printer(9);
          return false;
        }
        waitlist = dp->base.passwaits;
      } break;
      case VK_PASSTYPE::CP: waitlist = (( RenderGraph::CP_VK* )last_ptr)->base.passwaits; break;
      case VK_PASSTYPE::TP: waitlist = (( RenderGraph::TP_VK* )last_ptr)->base.passwaits; break;
      case VK_PASSTYPE::WP: waitlist = (( RenderGraph::WP_VK* )last_ptr)->base.passwaits; break;
      default: printer(6); return false;
    }
    ...
    for (unsigned int wait_i = 0; wait_i < waitlistsize; wait_i++) {
      ...
      switch (passwaitdesc_handle.type) {
        case VKHANDLETYPEs::WAITDESC_SUBDP: {
          RenderGraph::WaitDesc_SubDP* wait_subdp =
            RenderGraph::getDesc_fromHandle<RenderGraph::WaitDesc_SubDP>(passwaitdesc_handle);
          VKOBJHANDLE passhandle = *( VKOBJHANDLE* )wait_subdp->passhandle;
          if (passhandle.type != VKHANDLETYPEs::DRAWPASS) {
            printer(7);
          }
          ...
          if (wait_subdp->SubPassIndex >= dp->base.ALLSUBPASSESCOUNT) {
            printer(8);
            return false;
          }
        } break;
        case VKHANDLETYPEs::WAITDESC_SUBCP: {
          RenderGraph::WaitDesc_SubCP* wait_subcp =
            RenderGraph::getDesc_fromHandle<RenderGraph::WaitDesc_SubCP>(passwaitdesc_handle);
          VKOBJHANDLE passhandle = *( VKOBJHANDLE* )wait_subcp->passhandle;
          if (passhandle.type != VKHANDLETYPEs::COMPUTEPASS) {
            printer(7);
          }
          ...
          if (wait_subcp->SubPassIndex >= cp->base.ALLSUBPASSESCOUNT) {
            printer(8);
            return false;
          }
        } break;
        case VKHANDLETYPEs::WAITDESC_SUBTP: {
          RenderGraph::WaitDesc_SubTP* wait_subtp =
            RenderGraph::getDesc_fromHandle<RenderGraph::WaitDesc_SubTP>(passwaitdesc_handle);
          VKOBJHANDLE passhandle = *( VKOBJHANDLE* )wait_subtp->passhandle;
          if (passhandle.type != VKHANDLETYPEs::TRANSFERPASS) {
            printer(7);
          }
          ...
          if (wait_subtp->SubPassIndex >= tp->base.ALLSUBPASSESCOUNT) {
            printer(8);
            return false;
          }
        } break;
        default: printer(6); break;
      }
    }
...
unsigned char Finish_RenderGraphConstruction(subdrawpass_tgfx_handle IMGUI_Subpass) {
  ...
  if (!Check_WaitHandles()) {
    printer(result_tgfx_NOTCODED,
            "VulkanRenderer->Finish_RenderGraphConstruction() has failed but should return to old "
            "RenderGraph. This is not the case for now! Fixes: Return back to old rendergraph, "
            "destroy all vk objects created for deleted passes");
    RenderGraph::setRGSTATUS(
      RGReconstructionStatus::Invalid); // User can change only waits of a pass, so all rendergraph
  ...
  // So move all waits to a seperate memory block (so CPU can cache them)
  {
    ...
    for (current_PassID; current_PassID < RG_RECDATA::ID_GENERATOR.load(); current_PassID++) {
      ...
      switch (type) {
        case VK_PASSTYPE::DP: {
          RenderGraph::DP_VK* dp = (( RenderGraph::DP_VK* )last_ptr);
          if (current_PassID != dp->base.PASS_ID) {
            printer(9);
            return false;
          }
          dp->convert_waits();
        ...
        case VK_PASSTYPE::CP: {
          ...
          if (current_PassID != cp->base.PASS_ID) {
            printer(9);
            return false;
          }
          cp->convert_waits();
        ...
        case VK_PASSTYPE::TP: {
          ...
          if (current_PassID != tp->base.PASS_ID) {
            printer(9);
            return false;
          }
          tp->convert_waits();
        ...
        case VK_PASSTYPE::WP: {
          ...
          if (current_PassID != wp->base.PASS_ID) {
            printer(9);
            return false;
          }
          wp->convert_waits();
        } break;
        default: printer(15); return false;
      }
  ...
  if (imgui) {
    ...
    if (IMGUI_Subpass) {
      ...
      RTSLOTSET_VKOBJ*       SLOTSET =
        contentmanager->GETRTSLOTSET_ARRAY().getOBJbyINDEX(maindp->BASESLOTSET_ID);
      ...
      if (SLOTSET->PERFRAME_SLOTSETs[0].COLORSLOTs_COUNT != 1) {
        ...
        RenderGraph::setRGSTATUS(RGReconstructionStatus::Invalid); // User can delete a draw pass,
                                                                   // dear Imgui fails in this case.
      ...
      if (imgui->Get_IMGUIStatus() == imgui_vk::IMGUI_STATUS::UNINITIALIZED) {
        ...
        if (vkCreateDescriptorPool(rendergpu->LOGICALDEVICE(), &descpool_ci, nullptr,
                                   &imgui->descpool) != VK_SUCCESS) {
        ...
        if (!core_vk->GETWINDOWs().size()) {
          printer(result_tgfx_FAIL,
                  "Initialization of dear IMGUI has failed because you didn't create a window!");
          return result_tgfx_FAIL;
        }
        ...
        init_info.Queue =
          queuesys->get_queue(rendergpu, core_vk->GETWINDOWs()[0]->presentationqueue);
        init_info.QueueFamily =
          queuesys->get_queuefam_index(rendergpu, core_vk->GETWINDOWs()[0]->presentationqueue);
...
// WaitInfos is a pointer because function expects a list of waits (Color attachment output and also
// VertexShader-UniformReads etc)
static passwaitdescription_tgfx_handle CreatePassWait_SubDrawPass(
  drawpass_tgfx_handle* PassHandle, unsigned int SubDPIndex,
  waitsignaldescription_tgfx_handle* WaitInfos, unsigned char isLastFrame) {
  if (!PassHandle) {
    printer(result_tgfx_INVALIDARGUMENT,
            "CreatePassWait_SubDrawPass()'s first argument can't be nullptr!");
    return nullptr;
  }
  TGFXLISTCOUNT(core_tgfx_main, WaitInfos, waitlistsize);
  if (waitlistsize) {
    printer(result_tgfx_WARNING, "waitsignaldescription isn't supported for now!");
  }
  RenderGraph::WaitDesc_SubDP* waitdesc = ( RenderGraph::WaitDesc_SubDP* )VK_ALLOCATE_AND_GETPTR(
    VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(RenderGraph::WaitDesc_SubDP));
...
// WaitInfo is single, because function expects only one wait and it should be created with
// CreateWaitSignal_ComputeShader()
static passwaitdescription_tgfx_handle CreatePassWait_SubComputePass(
  computepass_tgfx_handle* PassHandle, unsigned int SubCPIndex,
  waitsignaldescription_tgfx_handle WaitInfo, unsigned char isLastFrame) {
  if (!PassHandle) {
    printer(result_tgfx_INVALIDARGUMENT,
            "CreatePassWait_SubComputePass()'s first argument can't be nullptr!");
    return nullptr;
  }
  RenderGraph::WaitDesc_SubCP* waitdesc = ( RenderGraph::WaitDesc_SubCP* )VK_ALLOCATE_AND_GETPTR(
    VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(RenderGraph::WaitDesc_SubCP));
  waitdesc->passhandle            = PassHandle;
...
// WaitInfo is single, because function expects only one wait and it should be created with
// CreateWaitSignal_Transfer()
static passwaitdescription_tgfx_handle CreatePassWait_SubTransferPass(
  transferpass_tgfx_handle* PassHandle, unsigned int SubTPIndex,
  waitsignaldescription_tgfx_handle WaitInfo, unsigned char isLastFrame) {
  if (!PassHandle) {
    printer(result_tgfx_INVALIDARGUMENT,
            "CreatePassWait_SubTransferPass()'s first argument can't be nullptr!");
    return nullptr;
  }
  RenderGraph::WaitDesc_SubTP* waitdesc = ( RenderGraph::WaitDesc_SubTP* )VK_ALLOCATE_AND_GETPTR(
    VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(RenderGraph::WaitDesc_SubTP));
  waitdesc->passhandle            = PassHandle;
...
static passwaitdescription_tgfx_handle CreatePassWait_WindowPass(
  windowpass_tgfx_handle* PassHandle) {
...
static subdrawpassdescription_tgfx_handle CreateSubDrawPassDescription(
  passwaitdescription_tgfx_listhandle waitdescs, inheritedrtslotset_tgfx_handle irtslotset,
  subdrawpassaccess_tgfx WaitOP, subdrawpassaccess_tgfx ContinueOP) {
  RG_RECDATA::SubDPDESC* desc = ( RG_RECDATA::SubDPDESC* )VK_ALLOCATE_AND_GETPTR(
    VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(RG_RECDATA::SubDPDESC));
  if (irtslotset) {
    desc->IRTSLOTSET_ID = contentmanager->GETIRTSLOTSET_ARRAY().getINDEXbyOBJ(
      contentmanager->GETIRTSLOTSET_ARRAY().getOBJfromHANDLE(*( VKOBJHANDLE* )&irtslotset));
  } else {
    desc->IRTSLOTSET_ID = UINT32_MAX;
  }
  desc->WAITOP     = WaitOP;
...
static subcomputepassdescription_tgfx_handle CreateSubComputePassDescription(
  passwaitdescription_tgfx_listhandle waitdescs) {
  RG_RECDATA::SubCPDESC* desc = ( RG_RECDATA::SubCPDESC* )VK_ALLOCATE_AND_GETPTR(
    VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(RG_RECDATA::SubCPDESC));
  desc->waits = waitdescs;

  VKDATAHANDLE handle = RG_RECDATA::getHandle_ofDesc(desc);
  return *( subcomputepassdescription_tgfx_handle* )&handle;
}
static subtransferpassdescription_tgfx_handle CreateSubTransferPassDescription(
  passwaitdescription_tgfx_listhandle waits, transferpasstype_tgfx type_i) {
#ifdef TURAN_DEBUGGING
  if (type_i > transferpasstype_tgfx_COPY) {
    printer(21);
  }
#endif
  RG_RECDATA::SubTPDESC* desc = ( RG_RECDATA::SubTPDESC* )VK_ALLOCATE_AND_GETPTR(
    VKGLOBAL_VIRMEM_CURRENTFRAME, sizeof(RG_RECDATA::SubTPDESC));
  desc->waits = waits;
  desc->type  = type_i;
...
waitsignaldescription_tgfx_handle waitsignal_cs(unsigned char fds, unsigned char sdf,
                                                unsigned char lsdfv) {
  return nullptr;
}
waitsignaldescription_tgfx_handle waitsignal_drawindirectconsume() { return nullptr; }
waitsignaldescription_tgfx_handle waitsignal_tr(unsigned char dslkþfk, unsigned char sdfsd) {
  return nullptr;
}
