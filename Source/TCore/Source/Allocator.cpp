...
TCSuperBlock* allocateSuperMemoryBlock(unsigned long long blockSize, const char* superMemBlockName)
{
...
struct EndOfPageAllocatorServices
{
	...
	static void* Malloc(TCSuperBlock* memBlock, unsigned int size, const char* name)
	{
...
struct Vector
{
	...
	// Data allocated right after these
};
...
void destroyElements(void* data, uint64_t destroyCount)
{
	...
	{ // Or set zero to memory then decommit it
...
void addElements(void* data, uint64_t elementCount)
{
	...
	tlCommit(reinterpret_cast<void*>(d + (hnd->elementSize * hnd->count)), elementCount * hnd->elementSize);
...
void* tlVectorCreateFnc(TCSuperBlock* memblock,
						unsigned int elementSize,
						unsigned int initialSize,
						unsigned int maxSize,
						tlVectorFlag vectorFlag,
						...)
{
	...
	void* alloc =
		(void*)allocateFromSuperMemoryBlock(memblock, sizeof_vectorFuncs + sizeof(TCVector) + (elementSize * maxSize));
	tlCommit(alloc, sizeof_vectorFuncs + sizeof(TCVector));
	TCVector* vector = reinterpret_cast<TCVector*>(reinterpret_cast<uintptr_t>(alloc) + sizeof_vectorFuncs);
...
unsigned char tlVectorEraseFnc(TCVectorHandle data, unsigned int elementIndex)
{
	TCVector* hnd = getVectorTapi(data);
...
tlAllocator* initialize()
{
	...
	firstPageMemBlockCount = (GPageSize - sizeof(TCSuperBlock::superBlockInfo)) / sizeof(TCSuperBlock::memBlock);
