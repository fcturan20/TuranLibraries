...
struct baseCompManager
{
	...
	struct baseCOMPTYPE
	{
		unsigned int (*getDirX)(componentHnd_ecstapi comp), (*getDirY)(componentHnd_ecstapi comp),
			(*getDirZ)(componentHnd_ecstapi comp);
	...
	static componentHnd_ecstapi createComp()
	{
		...
		return reinterpret_cast<componentHnd_ecstapi>(
			&v_BASECOMPs[allocatorSys->vector_manager->size(v_BASECOMPs) - 1]);
	}
	static void destroyComp(componentHnd_ecstapi) {}
	static compTypeHnd_ecstapi getCompType(const char* name) { return reinterpret_cast<compTypeHnd_ecstapi>(&type); }
	...
	static unsigned int getDir(componentHnd_ecstapi comp) { return ((baseCOMPSTRUCT*)comp)->dir[2]; };
	static float getSpeed(componentHnd_ecstapi comp) { return ((baseCOMPSTRUCT*)comp)->speed; };
...
struct derivedCompManager
{
	...
	static unsigned int getDir(componentHnd_ecstapi comp) { return ((derivenCOMPSTRUCT*)comp)->time * 2; };
	static float getSpeed(componentHnd_ecstapi comp) { return ((derivenCOMPSTRUCT*)comp)->speed; };
	...
	static void initialize()
	{
		...
		baseType->getDirX = getDir;
		baseType->getDirY = getDir;
		baseType->getDirZ = getDir;
		baseType->getSpeed = getSpeed;
...
int main()
{
	...
	compTypeID_ecstapi baseCompTypeID =
		editorECS->addComponentType("BASE",
									reinterpret_cast<compType_ecstapi>(baseCompManager::type),
									baseCompManager::getECSCompManager(),
									nullptr,
									0);
	ecs_compTypePair pair;
	pair.base = baseCompTypeID;
	pair.overriden = (compType_ecstapi)derivedCompManager::baseType;
	compTypeID_ecstapi derivedCompTypeID =
		editorECS->addComponentType("DERIVED",
									reinterpret_cast<compType_ecstapi>(derivedCompManager::type),
									derivedCompManager::getECSCompManager(),
									&pair,
									1);
	...
	// Create an entity with base type and get value of the component
	{
		...
		componentHnd_ecstapi baseComp =
			editorECS->get_component_byEntityHnd(baseEntity, baseCompTypeID, &overridenCompType);
		printf("Base Component DirZ: %d \n",
			   reinterpret_cast<baseCompManager::baseCOMPTYPE*>(overridenCompType)->getDirZ(baseComp));
	}

	// Create an entity with derived type and get value of the component
	{
		...
		compTypeHnd_ecstapi overridenCompType;
		componentHnd_ecstapi baseComp =
			editorECS->get_component_byEntityHnd(derivenEntity, baseCompTypeID, &overridenCompType);
		printf("Derived Component DirZ: %d \n",
			   reinterpret_cast<baseCompManager::baseCOMPTYPE*>(overridenCompType)->getDirZ(baseComp));
		compTypeHnd_ecstapi derivedOverridenCompType;
		componentHnd_ecstapi derivedComp =
			editorECS->get_component_byEntityHnd(derivenEntity, derivedCompTypeID, &derivedOverridenCompType);
		printf("Derived Component SpeedXTime: %d \n",
			   reinterpret_cast<derivedCompManager::derivenCOMPTYPE*>(derivedOverridenCompType)
				   ->getSpeedXTime(derivedComp));
	}
