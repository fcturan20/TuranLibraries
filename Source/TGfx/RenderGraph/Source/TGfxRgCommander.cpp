...
static void initializeImGuiCommandBufferAndFence()
{
	// CommandPool Gen
	{
		...
		ThrowIfFailed(vkCreateCommandPool(rendergpu->LOGICALDEVICE(), &ci, nullptr, &hidden->ImGuiCommandPool),
	...
	// Allocate command buffers
	{
		...
		ThrowIfFailed(vkAllocateCommandBuffers(rendergpu->LOGICALDEVICE(), &ai, hidden->ImGuiCommandBuffer),
	...
	for (uint8_t i = 0; i < 2; i++)
	{
		...
		ThrowIfFailed(vkCreateFence(rendergpu->LOGICALDEVICE(), &ci, nullptr, &hidden->ImGuiCBFences[i]),
	...
	for (uint8_t i = 0; i < 2; i++)
	{
		...
		VkAttachmentDescription at_d = {};
		{
			...
			at_d.flags = 0;
			auto texture =
				contentmanager->GETTEXTURES_ARRAY().getOBJbyINDEX(core_vk->GETWINDOWs()[0]->Swapchain_Textures[i]);
	...
	for (uint8_t i = 0; i < 2; i++)
	{
		...
		ThrowIfFailed(vkCreateFramebuffer(rendergpu->LOGICALDEVICE(), &ci, nullptr, &hidden->ImGuiFBs[i]),
					  "ImGui FB gen failed!");
	}
}
struct renderer_funcs
{
	...
	static void Run()
	{
		...
		if (VKCONST_onlyRenderImGui)
		{
			...
			ThrowIfFailed(
				vkWaitForFences(rendergpu->LOGICALDEVICE(), 1, &hidden->ImGuiCBFences[frame_i], VK_TRUE, UINT64_MAX),
	...
	static void DrawDirect(struct tgfx_buffer* VertexBuffer_ID,
						   struct tgfx_buffer* IndexBuffer_ID,
						   unsigned int Count,
	...
	static void SwapBuffers(struct tgfx_window* WindowHnd, windowpass_tgfx_handle WindowPassHnd)
	{
		...
		vkAcquireNextImageKHR(rendergpu->LOGICALDEVICE(),
							  VKWINDOW->Window_SwapChain,
							  UINT64_MAX,
							  semaphore.vksemaphore(),
							  fence.Fence_o,
							  &SwapchainImage_Index);
	...
	static void FindBufferOBJ_byBufType(struct tgfx_buffer* Handle, VkBuffer& TargetBuffer, VkDeviceSize& TargetOffset)
	{
...
{
	return renderer->Get_FrameIndex(false);
}
