...
void loadPlugins()
{
	...
	auto stringSysType = (STRINGSYS_TAPI_PLUGIN_LOAD_TYPE)editorECS->getSystem(STRINGSYS_TAPI_PLUGIN_NAME);
	...
	pluginElement* v_pluginElements =
		(pluginElement*)allocatorSys->vectorManager->create(superMemBlock, sizeof(pluginElement), 10, 1000, 0);
...
textureUsageMask_tgfxflag textureAllUsages = textureUsageMask_tgfx_COPYFROM | textureUsageMask_tgfx_COPYTO |
											 textureUsageMask_tgfx_RANDOMACCESS | textureUsageMask_tgfx_RASTERSAMPLE |
											 textureUsageMask_tgfx_RENDERATTACHMENT,
						  storageImageUsage = textureUsageMask_tgfx_COPYTO | textureUsageMask_tgfx_RANDOMACCESS;
...
void keyCB(struct tgfx_window* windowHnd,
		   void* userPointer,
		   key_tgfx key,
		   int scanCode,
		   keyAction_tgfx action,
		   keyMod_tgfx mode)
{
	if (key == key_tgfx_A)
	{
...
int frameIndx = 0;
void closeCB(struct tgfx_window* windowHnd, void* userPtr)
{
	frameIndx = 1000000;
}
...
void createBuffersAndTextures()
{
	...
	contentManager->createHeap(
		gpu, gpuDesc.memRegions[hostVisibleMemType].memoryTypeId, heapSize, 0, nullptr, &hostVisibleHeap);
	contentManager->mapHeap(hostVisibleHeap, 0, heapSize, 0, nullptr, &mappedRegion);

	contentManager->createHeap(
		gpu, gpuDesc.memRegions[deviceLocalMemType].memoryTypeId, heapSize, 0, nullptr, &deviceLocalHeap);
	...
	textureDesc.usage =
		textureUsageMask_tgfx_RENDERATTACHMENT | textureUsageMask_tgfx_COPYFROM | textureUsageMask_tgfx_COPYTO;
	tgfx_uvec4 maxSize = {};
	if (tgfx->helpers->getTextureTypeLimits(textureDesc.dimension,
											textureDesc.dataOrder,
											textureDesc.channelType,
											textureDesc.usage,
											gpu,
											&maxSize.x,
											&maxSize.y,
	...
	void* reiChikitaData =
		stbi_load(SOURCE_DIR "/shaders/reiChikita.jpg", &reiChikitaWHC.x, &reiChikitaWHC.y, &reiChikitaWHC.z, 4);
	...
	auto calculateHeapOffset = [&lastMemPoint](const tgfx_heapRequirementsInfo& heapReq) -> void {
		lastMemPoint = ((lastMemPoint / heapReq.offsetAlignment) + ((lastMemPoint % heapReq.offsetAlignment) ? 1 : 0)) *
...
tgfx_bindingTableDescription bufferBindingType = {}, textureBindingType = {}, samplerBindingType = {};
struct tgfx_pipeline* firstComputePipeline = {};
struct tgfx_pipeline* firstRasterPipeline = {};
struct tgfx_bindingTable *bufferBindingTable = {}, *textureBindingTable = {}, *samplerBindingTable = {};
...
void compileShadersandPipelines()
{
	// Compile compute shader, create binding table type & compute pipeline
	{
		const char* shaderText =
			(const char*)fileSys->readText(tlStringUTF8, SOURCE_DIR "/shaders/firstComputeShader.comp", tlStringUTF8);
		struct tgfx_shaderSource* firstComputeShader = nullptr;
		contentManager->compileShaderSource(gpu,
											shaderlanguages_tgfx_GLSL,
											shaderStage_tgfx_COMPUTESHADER,
											(void*)shaderText,
											strlen(shaderText),
											&firstComputeShader);
		...
		// Create binding table types
		{
			...
			desc.visibleStagesMask =
				shaderStage_tgfx_COMPUTESHADER | shaderStage_tgfx_VERTEXSHADER | shaderStage_tgfx_FRAGMENTSHADER;
		...
		contentManager->createComputePipeline(firstComputeShader, 1, &bufferBindingType, 0, 0, &firstComputePipeline);
	}
	...
	// Compile first vertex-fragment shader & raster pipeline
	{
		...
		const char* vertShaderText =
			(const char*)fileSys->readText(tlStringUTF8, SOURCE_DIR "/shaders/firstShader.vert", tlStringUTF8);
		...
		const char* fragShaderText =
			(const char*)fileSys->readText(tlStringUTF8, SOURCE_DIR "/shaders/firstShader.frag", tlStringUTF8);
		...
		tgfx_bindingTableDescription bindingTypes[3] = {bufferBindingType, textureBindingType, samplerBindingType};
	...
	// Instantiate binding tables
	{
		...
		contentManager->setBindingTable_Buffer(
			bufferBindingTable, 1, &bindingIndex, &firstBuffer, nullptr, nullptr, 0, nullptr);

		contentManager->createBindingTable(gpu, &textureBindingType, &textureBindingTable);
		contentManager->setBindingTable_Texture(textureBindingTable, 1, &bindingIndex, &reiChikitaTexture);
...
void recordCommandBundles()
{
	...
	tgfxRenderer->cmdBarrierTexture(initBundle,
									0,
									reiChikitaTexture,
									...
									0,
									nullptr);
	tgfxRenderer->cmdCopyBufferToTexture(
		initBundle, 1, firstBuffer, 2048, reiChikitaTexture, image_access_tgfx_TRANSFER_DIST, 0, nullptr);
	tgfxRenderer->cmdBarrierTexture(initBundle,
									2,
									reiChikitaTexture,
									image_access_tgfx_TRANSFER_DIST,
	...
	{
		struct tgfx_bindingTable* bindingTables[3] = {bufferBindingTable, textureBindingTable, samplerBindingTable};
		uint32_t cmdKey = 0;
		...
		tgfxRenderer->cmdBindBindingTables(standardDrawBundle, cmdKey++, 0, 3, bindingTables, pipelineType_tgfx_RASTER);
		indirectOperationType_tgfx firstOp[5] = {indirectOperationType_tgfx_DRAWNONINDEXED,
												 indirectOperationType_tgfx_DRAWNONINDEXED,
												 indirectOperationType_tgfx_DRAWNONINDEXED,
												 indirectOperationType_tgfx_DRAWNONINDEXED,
												 indirectOperationType_tgfx_DRAWNONINDEXED};
		tgfxRenderer->cmdExecuteIndirect(
			standardDrawBundle, cmdKey++, 5, firstOp, firstBuffer, sizeof(firstUboStruct), 0, nullptr);
...
void recordSwpchnCmdBundles()
{
	for (uint32_t i = 0; i < swapchainTextureCount; i++)
	{
		...
		// Record command bundle
		perSwpchnCmdBundles[i] = tgfxRenderer->beginCommandBundle(gpu, cmdCount, firstRasterPipeline, 0, nullptr);
		{
			uint32_t cmdKey = 0;

			tgfxRenderer->cmdBindPipeline(perSwpchnCmdBundles[i], cmdKey++, firstRasterPipeline);
			tgfxRenderer->cmdBindBindingTables(
				perSwpchnCmdBundles[i], cmdKey++, 0, 1, &bufferBindingTable, pipelineType_tgfx_COMPUTE);
	...
	*/
	...
	while (++frameIndx < 10000)
	{
		...
		// Record frame's command buffer
		{
			...
			tgfxRenderer->beginRasterpass(frameCmdBuffer, 1, &colorAttachmentInfo, &depthAttachmentInfo, 0, nullptr);
