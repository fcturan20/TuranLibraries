...
vkext_dynamicRendering::vkext_dynamicRendering(GPU_VKOBJ* gpu) : vkext_interface(gpu, nullptr, &features)
{
...
void vkext_dynamicRendering::inspect()
{
	...
	{
		m_gpu->ext()->m_activeDevExtNames[m_gpu->ext()->m_devExtCount++] = (VK_KHR_CREATE_RENDERPASS_2_EXTENSION_NAME);
		m_gpu->ext()->m_activeDevExtNames[m_gpu->ext()->m_devExtCount++] =
			(VK_KHR_DEPTH_STENCIL_RESOLVE_EXTENSION_NAME);
		m_gpu->ext()->m_activeDevExtNames[m_gpu->ext()->m_devExtCount++] = (VK_KHR_MULTIVIEW_EXTENSION_NAME);
		m_gpu->ext()->m_activeDevExtNames[m_gpu->ext()->m_devExtCount++] = (VK_KHR_MAINTENANCE2_EXTENSION_NAME);
		m_gpu->ext()->m_activeDevExtNames[m_gpu->ext()->m_devExtCount++] = (VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME);
...
void vkext_beginDynamicRenderPass(VkCommandBuffer cb,
								  ...
								  tgfx_rasterpassBeginSlotInfo depthAttachment)
{
	...
	if (depthAttachment.texture)
	{
		attachmentInfos[colorAttachmentCount].imageView = getOBJ<TEXTURE_VKOBJ>(depthAttachment.texture)->vk_imageView;
		VkAccessFlags unused = {};
