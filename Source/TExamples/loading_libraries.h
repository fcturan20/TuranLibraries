...
extern struct tgfx_texture *swapchain_textures[2] = {NULL, NULL}, depthbuffer_handle = NULL, orange_texture = NULL;
...
public:
	...
	static void Load_AllLibraries()
	{
		...
		if (!loader)
		{
			printf("Registry System DLL doesn't contain load_plugin function, which it should. You are "
		...
		VIRTUALMEMORY_TAPI_PLUGIN_TYPE virmem =
			(VIRTUALMEMORY_TAPI_PLUGIN_TYPE)regsys->get("tapi_virtualmemsys", MAKE_PLUGIN_VERSION_TAPI(0, 0, 0), 0);
		if (!virmem)
		{
			printf("Registry System should have a virtual memory system, but it doesn't. You are probably "
		...
		if (!unittestloader)
		{
			printf("UnitTest System DLL doesn't contain load_plugin function, which it should. You are "
				   "probably using a newer version that uses different load scheme!");
		}
		...
		if (!aosloader)
		{
			printf("Array of Strings System DLL doesn't contain load_plugin function, which it should. You "
		...
		if (!unittestloader)
		{
			printf("ThreadingJob System DLL doesn't contain load_plugin function, which it should. You are "
				   "probably using a newer version that uses different load scheme!");
		}
		...
		if (!unittestloader)
		{
			printf("File System DLL doesn't contain load_plugin function, which it should. You are probably "
		...
		if (!loggerloader)
		{
			printf("Logger System DLL doesn't contain load_plugin function, which it should. You are probably "
				   "using a newer version that uses different load scheme!");
		}
		...
		if (!bitsetloader)
		{
			printf("Bitset System DLL doesn't contain load_plugin function, which it should. You are probably "
				   "using a newer version that uses different load scheme!");
		}
		...
		{
			printf("Profiler System DLL doesn't contain load_plugin function, which it should. You are "
				   "probably using a newer version that uses different load scheme!");
		}
		...
		if (!tgfxloader)
		{
			printf("TGFX System DLL doesn't contain load_plugin function, which it should. You are probably "
				   "using a newer version that uses different load scheme!");
		}
											...
											uvec2_tgfx window_resolution)
	{
		...
		TGFXHELPER->getGPUInfo_General(gpulist[0],
									   &gpuname,
									   nullptr,
									   nullptr,
									   nullptr,
		...
		initSecondStageInfo_tgfxhnd secondinfo = TGFXHELPER->Create_GFXInitializationSecondStageInfo(
			gpulist[0], 100, 1000, 1000, 1000, 1000, true, (extension_tgfxlsthnd)(&TGFXCORE->INVALIDHANDLE));
		...
		TURANLIBRARY_LOADER::printf_log_tgfx(result_tgfx_SUCCESS,
											 (std::string("Monitor Count: ") + std::to_string(monitorcount)).c_str());
		textureUsageFlag_tgfxhnd swapchaintexture_usageflag =
			TGFXHELPER->createUsageFlag_Texture(false, true, true, true, false);
		TGFXCORE->createWindow(window_resolution.x,
							   window_resolution.y,
							   monitorlist[0],
							   ...
							   nullptr,
							   &firstwindow);
		TGFXCORE->createSwapchain(firstwindow,
								  windowpresentation_tgfx_FIFO,
								  windowcomposition_tgfx_OPAQUE,
								  colorspace_tgfx_sRGB_NONLINEAR,
								  texture_channels_tgfx_BGRA8UNORM,
								  swapchaintexture_usageflag,
								  swapchain_textures);
	}
	...
	static void ReconstructRenderGraph_Simple()
	{
		...
		passwaitdescription_tgfx_handle wait_for_subcp[2] = {
			TGFXHELPER->CreatePassWait_SubComputePass(
				&ComputebeforeRasterization_CP, 0, TGFXHELPER->CreateWaitSignal_ComputeShader(true, true, true), false),
			(passwaitdescription_tgfx_handle)TGFXCORE->INVALIDHANDLE};
		...
		passwaitdescription_tgfx_handle wait_for_raster[2] = {
			TGFXHELPER->CreatePassWait_SubDrawPass(&framerendering_DP, 0, framerendering_waitsignals, false),
			(passwaitdescription_tgfx_handle)TGFXCORE->INVALIDHANDLE};
		subtransferpassdescription_tgfx_handle subtpdescs[5] = {
			TGFXHELPER->CreateSubTransferPassDescription(
				(passwaitdescription_tgfx_listhandle)(&TGFXCORE->INVALIDHANDLE), transferpasstype_tgfx_COPY),
		...
		RTSlotDescription_tgfxhnd rtslot_descs[3] = {
			TGFXHELPER->CreateRTSlotDescription_Color(swapchain_textures[0],
													  ...
													  0,
													  white),
			TGFXHELPER->CreateRTSlotDescription_DepthStencil(depthbuffer_handle,
															 depthbuffer_handle,
															 operationtype_tgfx_READ_AND_WRITE,
															 drawpassload_tgfx_CLEAR,
		...
		rtslotusage_tgfx_handle rtslotusages[3] = {
			TGFXHELPER->CreateRTSlotUsage_Color(
				rtslot_descs[0], operationtype_tgfx_READ_AND_WRITE, drawpassload_tgfx_CLEAR),
			TGFXHELPER->CreateRTSlotUsage_Depth(rtslot_descs[1],
												operationtype_tgfx_READ_AND_WRITE,
												drawpassload_tgfx_CLEAR,
												operationtype_tgfx_UNUSED,
												drawpassload_tgfx_CLEAR),
		...
		subdrawpassdescription_tgfx_handle subdp_descs[2] = {
			TGFXHELPER->CreateSubDrawPassDescription(
				subdp_waits, irtslotset, subdrawpassaccess_tgfx_ALLCOMMANDS, subdrawpassaccess_tgfx_ALLCOMMANDS),
