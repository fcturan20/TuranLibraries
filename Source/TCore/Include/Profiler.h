...
#define PROFILER_TAPI_PLUGIN_LOAD_TYPE const struct TCProfiler*

typedef struct TCProfiledScope TCProfiledScope;
typedef struct TCProfiler
{
	const struct TCProfilerData* data;
	TCProfiledScope* (*start)(const char* name, unsigned long long* duration, unsigned char timingType);
...
#define TURAN_PROFILE_SCOPE_NAS(profilertapi, name, duration_ptr)                                                      \
	TCProfiledScope* profile##__LINE__ = profilertapi->start(name, duration_ptr, 0)
#define TURAN_PROFILE_SCOPE_MCS(profilertapi, name, duration_ptr)                                                      \
	TCProfiledScope* profile##__LINE__ = profilertapi->start(name, duration_ptr, 1)
#define TURAN_PROFILE_SCOPE_MLS(profilertapi, name, duration_ptr)                                                      \
	TCProfiledScope* profile##__LINE__ = profilertapi->start(name, duration_ptr, 2)
#define TURAN_PROFILE_SCOPE_SEC(profilertapi, name, duration_ptr)                                                      \
	TCProfiledScope* profile##__LINE__ = profilertapi->start(name, duration_ptr, 3)
