...
static unsigned char vk_getTextureTypeLimits(texture_dimensions_tgfx dims,
											 ...
											 unsigned int* MAXMIPLEVEL)
{
	...
	if (vkGetPhysicalDeviceImageFormatProperties(GPU->vk_physical,
												 vk_findFormatVk(chnnltype),
												 vk_findImageTypeVk(dims),
												 Find_VkTiling(dataorder),
												 flag,
												 0,
												 &props) != VK_SUCCESS)
	{
