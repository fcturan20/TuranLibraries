...
void SetBit_True(bitset_tapi* set, unsigned int index) {
  if (index / 8 > GetBitset(set)->ByteLength - 1) {
    std::cout << "There is no such bit, maximum bit index: " << (GetBitset(set)->ByteLength * 8) - 1
              << std::endl;
    return;
  }
...
unsigned int tapi_GetByte_Length(const bitset_tapi* set) { return GetBitset(set)->ByteLength; }
