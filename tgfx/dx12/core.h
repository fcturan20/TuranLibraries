...
struct GPU_VKOBJ {
 private:
  ...
  unsigned int         TRANSFERs_supportedqueuecount = 0, COMPUTE_supportedqueuecount = 0,
               GRAPHICS_supportedqueuecount = 0;
...
struct window_dx {
  ...
  // Don't make these 3 again! There should 3 textures to be able to use 3 semaphores, otherwise 2
  // different semaphores for the same texture doesn't work as you expect! Presentation Semaphores
  // should only be used for rendergraph submission as wait
