...
/**	INTRODUCTION
 * This .cpp is core of the framegraph algorithm. Reconstruction, Execution and Destruction lies here and each of them
 * is a Section. Start_RenderGraphConstruction() and Finish_RenderGraphConstruction() are the main functions of
 * Reconstruction Section. Execute_RenderGraph() is the main function of Execution Section. Destroy_RenderGraph() is the
 * main function of Destruction Section. Start by reading Reconstruction and Executing Sections to learn about the
 * algorithm Similar datas are called different in specific sections of the algorithm, so be aware of the
 * section/context you are at RenderGraph = FrameGraph in general, otherwise it is specified in the context RenderGraph
 * Algorithm is divided into 2 main steps; Static, Dynamic. Static doesn't consider workload of the passes, only
 * considers manually defined relations. Dynamic considers workload of the passes and run (last, current and next frame)
 * passes Static information is needed to speed up dynamic data creation and only changes in Reconstruction Section, so
 * only calculated at Reconstruction Section Dynamic information is calculated every frame (because pass workloads are
 * dynamic), so it's in Execution Section. For now; you can match Reconstruction Section -> Static Step, Execution ->
 * Dynamic Step. But I can't say anything about future
 */
...
unsigned char Finish_RenderGraphConstruction(subdrawpass_tgfx_handle IMGUI_Subpass)
{
	...
	if (imgui)
	{
		...
		if (IMGUI_Subpass)
		{
			...
			if (((drawpass_vk*)((subdrawpass_vk*)IMGUI_Subpass)->DrawPass)
...
result_tgfx Execute_RenderGraph()
{
	...
	framegraph_vk &Current_FrameGraph = framegraphsys.framegraphs[CurrentFrameIndex],
				  &Last_FrameGraph = framegraphsys.framegraphs[LastFrameIndex];
	...
	case RenderGraphStatus::FinishConstructionCalled:
		...
		renderer->RG_Status =
			RenderGraphStatus::HalfConstructed; // When reconstruction finishes, it is half constructed.
