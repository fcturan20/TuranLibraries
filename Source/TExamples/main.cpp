...
{
	std::cout << ((result == result_tgfx_NOTCODED || result == result_tgfx_FAIL)
					  ? ("A notcoded or failed GFX function is called: ")
					  : (""))
			  << Text << std::endl;
#ifdef _DEBUG
...
void ThreadedMain()
{
	...
	unsigned int APIVER = 0, DRIVERVER = 0;
	tgfx_gpu_types GPUTYPE;
	tgfx_memorytype_list MemoryAllocList;
	unsigned char isGraphicsSupported = 0, isComputeSupported = 0, isTransferSupported = 0;
	core->Helpers.GetGPUInfo_General(DeviceGPUs[0],
									 &NAME,
									 &APIVER,
									 &DRIVERVER,
									 &GPUTYPE,
	...
	{
		tgfx_memoryallocationtype ALLOCTYPE;
		unsigned long long MAXSIZE = UINT32_MAX;
	...
	std::cout << "Desc indexing support: " << unsigned int(core->Helpers.DoesGPUsupportsVKDESCINDEXING(DeviceGPUs[0]))
			  << std::endl;
	tgfx_initializationsecondstageinfo secondinitialize = core->Helpers.Create_GFXInitializationSecondStageInfo(
		DeviceGPUs[0], 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 1, nullptr);
	...
	core->CreateWindow(1280,
					   720,
					   Monitors[0],
					   tgfx_windowmode::WINDOWED,
					   "Main Window",
					   core->Helpers.CreateTextureUsageFlag(1, 1, 1, 1, 1),
					   WindowCallback,
					   nullptr,
					   SwapchainTextures,
					   &MainWindow);
