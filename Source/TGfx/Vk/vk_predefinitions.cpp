...
const TCProfiler* profilerSys = nullptr;
...
void vk_createBackendAllocator()
{
	// Allocate enough space for all vulkan backend
	{
		...
		vm->commit(VKCONST_VIRMEMSPACE_BEGIN, VKCONST_VIRMEMPAGESIZE * VKCONST_VIRMEM_MANAGERONLYPAGECOUNT);
...
void vk_virmem::free_page(uint32_t suballocation_startoffset)
{
	...
	void* free_address = (void*)((VKCONST_VIRMEMPAGESIZE * page_i) + uintptr_t(VKCONST_VIRMEMSPACE_BEGIN));
