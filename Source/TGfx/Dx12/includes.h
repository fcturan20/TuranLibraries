 ...
 public:
  threadlocal_vector(const threadlocal_vector& copy) {
    ...
    sizes_and_capacities =
      new unsigned long[(threadingsys) ? (threadingsys->thread_count() * 2) : 2];
    for (unsigned int thread_i = 0;
         thread_i < ((threadingsys) ? (threadingsys->thread_count()) : 1); thread_i++) {
      lists[thread_i] = new T[copy.sizes_and_capacities[(thread_i * 2) + 1]];
      for (unsigned int element_i = 0; element_i < copy.sizes_and_capacities[thread_i * 2];
           element_i++) {
  ...
  unsigned long size(unsigned int ThreadIndex = UINT32_MAX) {
    return sizes_and_capacities[(ThreadIndex != UINT32_MAX)
                                  ? (ThreadIndex)
                                  : ((threadingsys) ? (threadingsys->this_thread_index() * 2) : 0)];
  }
  void push_back(const T& ref, unsigned int ThreadIndex = UINT32_MAX) {
    const unsigned int thread_i =
      ((ThreadIndex != UINT32_MAX) ? (ThreadIndex)
                                   : ((threadingsys) ? (threadingsys->this_thread_index()) : 0));
  ...
  T* data(unsigned int ThreadIndex = UINT32_MAX) {
    return lists[(ThreadIndex != UINT32_MAX)
                   ? (ThreadIndex)
                   : ((threadingsys) ? (threadingsys->this_thread_index()) : 0)];
  }
  void clear(unsigned int ThreadIndex = UINT32_MAX) {
    sizes_and_capacities[(ThreadIndex != UINT32_MAX)
                           ? (ThreadIndex * 2)
                           : ((threadingsys) ? (threadingsys->this_thread_index() * 2) : 0)] = 0;
  }
