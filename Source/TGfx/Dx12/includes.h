...
class threadlocal_vector
{
	...
	inline void expand_if_necessary(unsigned int thread_i)
	{
		...
		{
			T* newlist = nullptr;
			if (sizes_and_capacities[thread_i * 2] > 0)
			{
				newlist = new T[sizes_and_capacities[(thread_i * 2) + 1] * 2];
			}
			else
			{
				newlist = new T;
			...
			if (sizes_and_capacities[(thread_i * 2) + 1] == 0)
			{
				sizes_and_capacities[(thread_i * 2) + 1] = 1;
			}
			else
			{
...
public:
	...
	{
		unsigned int threadCount = (gThreadingSystem) ? (gThreadingSystem->thread_count()) : 1;
		lists = new T*[threadCount];
		sizes_and_capacities = new unsigned long[(gThreadingSystem) ? (gThreadingSystem->thread_count() * 2) : 2];
		for (unsigned int thread_i = 0; thread_i < ((gThreadingSystem) ? (gThreadingSystem->thread_count()) : 1);
			 thread_i++)
		{
	...
	// This constructor allocates initial_sizes memory but doesn't add any element to it, so you
	// should use push_back()
	...
	// This constructor allocates initial_sizes * 2 memory and fills it with initial_sizes number of
	// ref objects
	...
	{
		return sizes_and_capacities[(ThreadIndex != UINT32_MAX)
										? (ThreadIndex)
										: ((gThreadingSystem) ? (gThreadingSystem->this_thread_index() * 2) : 0)];
	}
	void push_back(const T& ref, unsigned int ThreadIndex = UINT32_MAX)
	{
		const unsigned int thread_i =
			((ThreadIndex != UINT32_MAX) ? (ThreadIndex)
										 : ((gThreadingSystem) ? (gThreadingSystem->this_thread_index()) : 0));
	...
	{
		return lists[(ThreadIndex != UINT32_MAX) ? (ThreadIndex)
												 : ((gThreadingSystem) ? (gThreadingSystem->this_thread_index()) : 0)];
	}
	void clear(unsigned int ThreadIndex = UINT32_MAX)
	{
		sizes_and_capacities[(ThreadIndex != UINT32_MAX)
								 ? (ThreadIndex * 2)
								 : ((gThreadingSystem) ? (gThreadingSystem->this_thread_index() * 2) : 0)] = 0;
	}
	T& get(unsigned int ThreadIndex, unsigned int ElementIndex) { return lists[ThreadIndex][ElementIndex]; }
	T& operator[](unsigned int ElementIndex)
	{
		return lists[((gThreadingSystem) ? (gThreadingSystem->this_thread_index()) : 0)][ElementIndex];
	}
	void PauseAllOperations(std::unique_lock<std::mutex>& Locker) { Locker = std::unique_lock<std::mutex>(Sync); }
};
