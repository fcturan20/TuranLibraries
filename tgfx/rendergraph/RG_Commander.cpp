...
static void initializeImGuiCommandBufferAndFence() {
  // CommandPool Gen
  {
    ...
    ThrowIfFailed(
      vkCreateCommandPool(rendergpu->LOGICALDEVICE(), &ci, nullptr, &hidden->ImGuiCommandPool),
      "ImGui CommandPool gen failed!");
  }
  // Allocate command buffers
  {
    ...
    ThrowIfFailed(
      vkAllocateCommandBuffers(rendergpu->LOGICALDEVICE(), &ai, hidden->ImGuiCommandBuffer),
      "ImGui CommandBuffer alloc failed!");
  }
  // Create fences
  for (uint8_t i = 0; i < 2; i++) {
    ...
    ThrowIfFailed(
      vkCreateFence(rendergpu->LOGICALDEVICE(), &ci, nullptr, &hidden->ImGuiCBFences[i]),
      "ImGui Fence gen failed!");
  }
  // Create RPs
  for (uint8_t i = 0; i < 2; i++) {
    ...
    VkAttachmentDescription at_d = {};
    {
      ...
      at_d.flags       = 0;
      auto texture     = contentmanager->GETTEXTURES_ARRAY().getOBJbyINDEX(
            core_vk->GETWINDOWs()[0]->Swapchain_Textures[i]);
    ...
    VkSubpassDescription sp_d = {};
    {
      ...
      at_r.attachment              = 0;
      at_r.layout                  = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
    ...
    ci.subpassCount = 1;
    ThrowIfFailed(vkCreateRenderPass(rendergpu->LOGICALDEVICE(), &ci, nullptr, &hidden->ImGuiRP[i]),
                  "ImGui RenderPass gen failed!");
  }
  // Create FBs
  for (uint8_t i = 0; i < 2; i++) {
    ...
    ThrowIfFailed(
      vkCreateFramebuffer(rendergpu->LOGICALDEVICE(), &ci, nullptr, &hidden->ImGuiFBs[i]),
      "ImGui FB gen failed!");
  }
}
struct renderer_funcs {
  ...
  static void Run() {
    ...
    if (VKCONST_onlyRenderImGui) {
      ...
      ThrowIfFailed(vkWaitForFences(rendergpu->LOGICALDEVICE(), 1, &hidden->ImGuiCBFences[frame_i],
                                    VK_TRUE, UINT64_MAX),
                    "waitingForFence failed!");
      ...
      // CB begin
      {
        ...
        bi.pInheritanceInfo         = {};
        ThrowIfFailed(vkBeginCommandBuffer(hidden->ImGuiCommandBuffer[frame_i], &bi),
                      "beginningCB failed!");
      }
      {
        ...
        clear_value.color.uint32[0] = 255;
        clear_value.color.uint32[1] = 255;
        clear_value.color.uint32[2] = 255;
      ...
      // As the name says, this function stops the thread while waiting for render calls
      // (raster-compute-transfer-barrier calls). With this way, we are sure that no command buffer
      // execution will collide so we can change descriptor sets according to that.
  ...
  static void DrawDirect(struct tgfx_buffer* VertexBuffer_ID, struct tgfx_buffer* IndexBuffer_ID,
    ...
    VKOBJHANDLE            ib_handle = *( VKOBJHANDLE* )&IndexBuffer_ID,
                vb_handle            = *( VKOBJHANDLE* )&VertexBuffer_ID;
    ...
    GRAPHICSPIPELINETYPE_VKOBJ* MATTYPE =
      contentmanager->GETGRAPHICSPIPETYPE_ARRAY().getOBJbyINDEX(PI->PROGRAM);
    ...
    if (IB) {
      ...
      if (VB) {
        ...
        call.VBuffer =
          gpu_allocator->get_memorybufferhandle_byID(rendergpu, VB->Block.MemAllocIndex);
      } else {
  ...
  static void SwapBuffers(struct tgfx_window* WindowHandle, windowpass_tgfx_handle WindowPassHandle) {
    ...
    if (SwapchainImage_Index != VKWINDOW->CurrentFrameSWPCHNIndex) {
      ...
      ErrorStream << "Vulkan's reported SwapchainImage_Index: " << SwapchainImage_Index
                  << "\nGFX's stored SwapchainImage_Index: "
                  << unsigned int(VKWINDOW->CurrentFrameSWPCHNIndex)
  ...
  static void FindBufferOBJ_byBufType(struct tgfx_buffer* Handle, VkBuffer& TargetBuffer,
    ...
    switch (objhandle.type) {
      case VKHANDLETYPEs::GLOBALSSBO: {
        ...
        TargetBuffer =
          gpu_allocator->get_memorybufferhandle_byID(rendergpu, obj->Block.MemAllocIndex);
      ...
      case VKHANDLETYPEs::INDEXBUFFER: {
        ...
        TargetBuffer =
          gpu_allocator->get_memorybufferhandle_byID(rendergpu, IB->Block.MemAllocIndex);
  ...
  static void CopyBuffer_toImage(subtransferpass_tgfx_handle TransferPassHandle,
    ...
    if (TargetTextureRegion.HEIGHT) {
      ...
      x.BufferImageCopy.imageExtent.height =
        std::floor(TEXTURE->HEIGHT / std::pow(2, TargetMipLevel));
    }
    if (TargetTextureRegion.WIDTH) {
      x.BufferImageCopy.imageExtent.width = TargetTextureRegion.WIDTH;
    } else {
      x.BufferImageCopy.imageExtent.width =
        std::floor(TEXTURE->WIDTH / std::pow(2, TargetMipLevel));
    }
    ...
    if (TEXTURE->CHANNELs == texture_channels_tgfx_D24S8) {
      x.BufferImageCopy.imageSubresource.aspectMask =
    ...
    if (TEXTURE->DIMENSION == texture_dimensions_tgfx_2DCUBE) {
      x.BufferImageCopy.imageSubresource.baseArrayLayer =
        Find_TextureLayer_fromtgfx_cubeface(TargetCubeMapFace);
    } else {
  ...
  static void ImageBarrier(subtransferpass_tgfx_handle BarrierTPHandle,
    ...
    RenderGraph::SubTPBARRIER_VK* tp =
      getPass_fromHandle<RenderGraph::SubTPBARRIER_VK>(subTPhandle);
  ...
  static void Dispatch_Compute(subcomputepass_tgfx_handle    ComputePassHandle,
    ...
    for (uint32_t i = 0; i < VKCONST_MAXDESCSET_PERLIST && ci->InstSETs[i] != UINT32_MAX; i++) {
      dc.BINDINGTABLEIDs[current_i++] = ci->InstSETs[i];
    }
    for (uint32_t j = 0; j < VKCONST_MAXDESCSET_PERLIST && ci->InstSETs[j] != UINT32_MAX; j++) {
      dc.BINDINGTABLEIDs[current_i++] = ci->InstSETs[j];
    }
...
bool RenderGraph::SubTPCOPY_VK::isWorkloaded() {
  if (!BUFBUFCopies.size() && !BUFIMCopies.size() && !IMIMCopies.size()) {
    return false;
  }
  return true;
}
bool RenderGraph::SubTPBARRIER_VK::isWorkloaded() {
  if (!this->BufferBarriers.size() && !this->TextureBarriers.size()) {
    return false;
  }
  return true;
}
...
bool RenderGraph::WP_VK::isWorkloaded() {
  if (WindowCalls.size() != UINT32_MAX) {
    return true;
  }
  return false;
}
...
bool RenderGraph::SubDP_VK::isThereWorkload() {
  ...
  if (!IndexedDrawCalls.size() && !NonIndexedDrawCalls.size()) {
    return false;
  }
  return true;
}
