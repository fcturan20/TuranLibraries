...
regsys_strings_array* create_array_of_string(void* reserved_virtualmemblock_ptr, long length_to_reserved_end)
{
	...
	unsigned int firstcommitsize =
		max(pagesize, initialelements_totalsize - (initialelements_totalsize % pagesize) + pagesize);
	...
	array->list_of_chars = (char*)array + (pagesize * (length_to_reserved_end / pagesize) /
										   (tapi_average_plugin_name_len / sizeof(unsigned int)));
...
{
	return (index < ((array->committed_ptrpagecount * pagesize) + (pagesize - sizeof(regsys_strings_array))) / 4)
			   ? (&array->list_of_chars[array->list_of_ptrs[index]])
			   : (NULL);
}
...
void add_plugin(const char* name, unsigned int version, void* plugin_ptr)
{
	...
	virmem_sys->virtual_commit(&regsys_d->plugin_list[regsys_d->plugin_capacity],
							   (regsys_d->plugin_capacity / pagesize) * pagesize);
	...
	regsys_d->plugin_capacity +=
		(regsys_d->plugin_capacity / (pagesize / sizeof(plugin_tapi_d))) * (pagesize / sizeof(plugin_tapi_d));
}
...
void* get_plugin(const char* name, unsigned int version, unsigned char search_flag)
{
	for (unsigned int i = 0; i < regsys_d->plugin_capacity; i++)
	{
		...
		if (name_ptr == UINT_MAX)
		{
			continue;
		}
		if (strcmp(&regsys_d->plugin_names->list_of_chars[name_ptr], name) == 0)
		{
			// printf("Plugin Major: %u, Input Major: %u",
			// GET_PLUGIN_VERSION_TAPI_MAJOR(regsys_d->plugin_list[i].plugin_version),
			// GET_PLUGIN_VERSION_TAPI_MAJOR(version));
			if (GET_PLUGIN_VERSION_TAPI_MAJOR(regsys_d->plugin_list[i].plugin_version) ==
					GET_PLUGIN_VERSION_TAPI_MAJOR(version) ||
				GET_PLUGIN_VERSION_TAPI_MAJOR(version) == 255)
			{
...
char** list_plugin(const char* name)
{
	if (name == NULL)
	{
		...
		for (unsigned int i = 0; i < regsys_d->plugin_capacity; i++)
		{
			...
			{
				unsigned long len =
					strlen(&regsys_d->plugin_names->list_of_chars[regsys_d->plugin_names->list_of_ptrs[i]]) + 1;
				list[validindex] = malloc(len);
				memcpy(list[validindex],
					   &regsys_d->plugin_names->list_of_chars[regsys_d->plugin_names->list_of_ptrs[i]],
					   len);
				validindex++;
			}
		}
...
// Loads both registrysys_tapi_type and virtualmemorysys_tapi_type (because plugin data storage is handled with virtual
// memory)
TCORE_FUNC_EXPORT void* load_plugin(registrysys_tapi* unused, unsigned char reload)
{
	...
	registry_system->funcs =
		(registrysys_tapi*)((char*)registry_system + sizeof(registrysys_tapi_type) + sizeof(registrysys_tapi_d));
	...
	regsys_d->plugin_capacity = (tapi_first_commit_size - sizeof(registrysys_tapi_type) - sizeof(registrysys_tapi_d) -
								 sizeof(registrysys_tapi)) /
								sizeof(plugin_tapi_d);
	...
	// printf("Length of plugin list array: %u", ((tapi_first_allocation_size - regsys_d->allocation_size_remaining) /
	// 16));
	regsys_d->plugin_names =
		create_array_of_string((char*)regsys_d + (((tapi_first_allocation_size / pagesize) * 15) / 16),
							   (((tapi_first_allocation_size / pagesize) * 15) / 16));
...
TCORE_FUNC_EXPORT void unload_plugin(registrysys_tapi* unused, unsigned char reload)
{
	...
	while (i < regsys_d->plugin_capacity)
	{
		if (regsys_d->plugin_list[i].plugin_ptr != NULL)
		{
			printf("Plugin named \" %s \" isn't unloaded!",
				   &regsys_d->plugin_names->list_of_chars[regsys_d->plugin_names->list_of_ptrs[i]]);
		}
	}
