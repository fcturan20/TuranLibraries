...
GPU_VKOBJ* core_public::getGPU(uint32_t i)
{
	return priv->m_gpus[i];
}
uint32_t core_public::gpuCount()
{
	return priv->m_gpus.size();
}
...
{
	bool Is_Found = false;
	for (uint32_t supported_extIndx = 0; supported_extIndx < VKGLOBAL_MAX_INSTANCE_EXT_COUNT; supported_extIndx++)
	{
...
void vk_createInstance()
{
	...
	{
		vkPrint(2, L"Vulkan Instance support more layer than backend has imagined, report this please!");
		return;
	}
...
void glfwWindowResizeCallback(GLFWwindow* glfwwindow, int width, int height)
{
	...
	vkwindow->m_resizeFnc(
		(struct tgfx_window*)vkwindow, vkwindow->m_userData, res, (struct tgfx_texture**)vkwindow->m_swapchainTextures);
}
...
{
	WINDOW_VKOBJ* vkwindow = (WINDOW_VKOBJ*)glfwGetWindowUserPointer(glfwWindow);
	vkwindow->m_keyFnc((struct tgfx_window*)vkwindow,
					   vkwindow->m_userData,
					   getKeyTgfx(key),
					   scan,
					   getKeyActionTgfx(action),
					   getKeyModTgfx(mode));
}
...
void vk_createWindow(const tgfx_windowDescription* desc, void* user_ptr, struct tgfx_window** window)
{
	...
	stringSys->convertString(tlStringUTF16, desc->name, tlStringUTF8, windowCName, VKCONST_MAX_WINDOW_NAME_CHAR - 1);
...
result_tgfx vk_createSwapchain(struct tgfx_gpu* gpu,
							   const tgfx_swapchainDescription* desc,
							   struct tgfx_texture** textures)
{
	...
	uint32_t queueFamIndxLst[VKCONST_MAXQUEUEFAMCOUNT_PERGPU] = {UINT32_MAX};
	{
		...
		if (swpchn_ci.imageUsage & (VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT))
		{
		...
		VK_getQueueAndSharingInfos(desc->permittedQueueCount,
								   desc->permittedQueues,
								   0,
								   nullptr,
								   queueFamIndxLst,
								   &swpchn_ci.queueFamilyIndexCount,
								   &swpchn_ci.imageSharingMode);
		swpchn_ci.pQueueFamilyIndices = queueFamIndxLst;

		if (vkCreateSwapchainKHR(GPU->vk_logical, &swpchn_ci, nullptr, &window->vk_swapchain) != VK_SUCCESS)
		{
	...
	{
		{
			...
			if (vkGetSwapchainImagesKHR(GPU->vk_logical, window->vk_swapchain, &created_imagecount, SWPCHN_IMGs) !=
				VK_SUCCESS)
			{
			...
			for (unsigned int i = 0; i < desc->imageCount; i++)
			{
				...
				if (vkCreateImageView(GPU->vk_logical, &ImageView_ci, nullptr, &SWPCHN_IMGVIEWs[i]) != VK_SUCCESS)
				{
...
result_tgfx vk_getCurrentSwapchainTextureIndex(struct tgfx_window* i_window, uint32_t* index)
{
	...
	if (vkAcquireNextImageKHR(window->m_gpu->vk_logical,
							  window->vk_swapchain,
							  UINT64_MAX,
							  window->vk_acquireSemaphore,
							  nullptr,
...
void vk_takeInputs()
{
	...
	for (uint32_t i = 0; i < priv->WINDOWs.size(); i++)
	{
		...
		{
			keyAction_tgfx curAction = getKeyActionTgfx(glfwGetMouseButton(vkwindow->vk_glfwWindow, mouseButtonIndx));
			...
			vkwindow->m_keyFnc(
				(struct tgfx_window*)vkwindow, vkwindow->m_userData, key, 0, curAction, keyMod_tgfx_NONE);
		}
	}
	/*
...
void vk_setInputMode(struct tgfx_window* windowHnd,
					 cursorMode_tgfx cursorMode,
					 unsigned char stickyKeys,
					 unsigned char stickyMouseButtons,
					 unsigned char lockKeyMods)
{
	...
	switch (cursorMode)
	{
	case cursorMode_tgfx_NORMAL: glfwSetInputMode(window->vk_glfwWindow, GLFW_CURSOR, GLFW_CURSOR_NORMAL); break;
	case cursorMode_tgfx_HIDDEN: glfwSetInputMode(window->vk_glfwWindow, GLFW_CURSOR, GLFW_CURSOR_HIDDEN); break;
...
result_tgfx vk_getWindow_GPUSupport(struct tgfx_window* i_window, struct tgfx_gpu* gpu, tgfx_windowGPUsupport* info)
{
	GPU_VKOBJ* GPU = getOBJ<GPU_VKOBJ>(gpu);
	WINDOW_VKOBJ* window = getOBJ<WINDOW_VKOBJ>(i_window);
	...
	vkGetPhysicalDeviceSurfaceFormatsKHR(GPU->vk_physical, window->vk_surface, &formatCount, VK_NULL_HANDLE);
	...
	if (vkGetPhysicalDeviceSurfaceFormatsKHR(GPU->vk_physical, window->vk_surface, &formatCount, formats) != VK_SUCCESS)
	{
	...
	vkGetPhysicalDeviceSurfacePresentModesKHR(GPU->vk_physical, window->vk_surface, &presentModesCount, VK_NULL_HANDLE);
	if (vkGetPhysicalDeviceSurfacePresentModesKHR(
			GPU->vk_physical, window->vk_surface, &presentModesCount, Presentations) != VK_SUCCESS)
	{
...
TGFX_BACKEND_ENTRY()
{
	return vk_load(ecsSys, core, printFnc);
}
...
VKAPI_ATTR VkBool32 VKAPI_CALL vk_debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT Message_Severity,
												...
												void* pUserData)
{
	...
	stringSys->convertString(tlStringUTF8,
...
void vk_setupDebugging()
{
	...
	if (vkCreateDebugUtilsMessengerEXT_loaded(VKGLOBAL_INSTANCE, &dbgMssngrCi, nullptr, &dbg_mssngr) != VK_SUCCESS)
	{
