...
namespace TuranEditor {
	...
	Resource_Identifier* Texture_Loader::Import_Texture(const char* PATH, const bool& flip_vertically, const GFX_API::Texture_Properties& Properties) {
		...
		if (data == nullptr) {
			TuranAPI::LOG_ERROR("Texture isn't loaded from source! Data is nullptr!");
			return nullptr;
		}
		switch (CHANNELs) {
		case 0:
			TuranAPI::LOG_ERROR("Texture has 0 channels, that's not possible! Loading failed!");
			return nullptr;
		case 1:
		...
		case 4:
			...
			break;
		default:
			TuranAPI::LOG_ERROR("Texture has unsupported number of channels, that's not possible! Loading failed!");
			return nullptr;
		}
		...
		texture_resource->BUF_VIS = GFX_API::BUFFER_VISIBILITY::CPUEXISTENCE_GPUREADONLY;
		...
		if (texture_resource) {
			...
			TuranAPI::LOG_ERROR("Texture isn't loaded!");
			return nullptr;
		}
