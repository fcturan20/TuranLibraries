...
static constexpr uint64_t maxVirmemPerSuperBlock = 1 << 24;  // Reserved address space size for each super block's internal usage
...
struct supermemoryblock_tapi {
  ...
  static constexpr uint32_t maxBlockCountPerSuperBlock = (maxVirmemPerSuperBlock - sizeof(superBlockInfo)) / sizeof(memBlock);
...
void freeSuperMemoryBlock(supermemoryblock_tapi* superMemBlock) {
  ...
  virtual_decommit((void*)(uintptr_t(superMemBlock) + pagesize), sizeof(supermemoryblock_tapi) - pagesize);
...
void* allocateFromSuperMemoryBlock(supermemoryblock_tapi* superMemBlock, unsigned int blockSize) {
  ...
  if (lastVirmemAddress + blockSize - reinterpret_cast<uintptr_t>(superMemBlock->info.ptr) >= superMemBlock->info.blockSize - pagesize) {
...
void* endofpage_malloc(supermemoryblock_tapi* memBlock, unsigned int size) {
  ...
  const unsigned long end_of_page_size = requestedAllocSize + (pagesize - (requestedAllocSize % pagesize)) + pagesize;

  void* base = allocateFromSuperMemoryBlock(memBlock, end_of_page_size);
  virtual_commit(base, end_of_page_size - pagesize);
  //Memory Layout: offsettedSpace (less than a page) - sizeVariable (unsigned int) - data (requestedSize) - reserved (non-committed) last page (page size)
  void* offsetted = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(base) + (pagesize - (requestedAllocSize % pagesize)));
...
void endofpage_free(supermemoryblock_tapi* memBlock, void* returnedAllocPTR) {
  ...
  if (!requestedAllocSize || requestedAllocSize == UINT32_MAX) { printf("End of Page allocator failed to free the allocation!"); return; }
  const unsigned long end_of_page_size = requestedAllocSize + (pagesize - (requestedAllocSize % pagesize)) + pagesize;
  void* base = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(returnedAllocPTR) - (pagesize - (requestedAllocSize % pagesize)));
...
vector_element_copy_func_tapi find_copyfunc(void* data) {
  ...
  if (hnd->flag & vector_flagbit_constructor_tapi) { loc -= sizeof(void*); }
...
vector_element_destructor_func_tapi find_destructor(void* data) {
  ...
  vector_tapi* hnd = getVectorTapi(data);
  if (hnd->flag & vector_flagbit_constructor_tapi) { loc -= sizeof(void*); }
  if (hnd->flag & vector_flagbit_copy_tapi) { loc -= sizeof(void*); }
...
void destroyElements(void* data, uint64_t destroyCount) {
  ...
  if (hnd->flag & vector_flagbit_destructor_tapi) {
    ...
    memset((unsigned char*)data + (hnd->elementSize * (hnd->count - destroyCount)), 0, hnd->elementSize * destroyCount);
    // TODO: This decommit fails sometimes, fix it
    // virtual_decommit((unsigned char*)data + (hnd->elementSize * (hnd->count - destroyCount)), hnd->elementSize * destroyCount);
  }
}
void addElements(void* data, uint64_t elementCount) {
  vector_tapi* hnd = getVectorTapi(data);
  uintptr_t d = reinterpret_cast<uintptr_t>(data);
  virtual_commit(reinterpret_cast<void*>(d + (hnd->elementSize * hnd->count)), elementCount * hnd->elementSize);
...
unsigned char resize(void* data, unsigned int newItemCount) {
  ...
  if (dif < 0) {
    destroyElements(data, -dif); return 1;
  }
  else if (dif > 0) {
...
void* create_vector(unsigned int elementSize, supermemoryblock_tapi* memblock, unsigned int initialSize, unsigned int maxSize, vector_flags_tapi vectorFlag, ...) {
  ...
  if (vectorFlag & vector_flagbit_constructor_tapi) { sizeof_vectorFuncs += sizeof(void*); }
  if (vectorFlag & vector_flagbit_copy_tapi) { sizeof_vectorFuncs += sizeof(void*); }
  if (vectorFlag & vector_flagbit_destructor_tapi) { sizeof_vectorFuncs += sizeof(void*); }
  if (vectorFlag & vector_flagbit_invalidator_tapi) { sizeof_vectorFuncs += sizeof(void*); }
  void* alloc = (void*)allocateFromSuperMemoryBlock(memblock, sizeof_vectorFuncs + sizeof(vector_tapi) + (elementSize * maxSize));
  virtual_commit(alloc, sizeof_vectorFuncs + sizeof(vector_tapi));
  vector_tapi* vector = reinterpret_cast<vector_tapi*>(reinterpret_cast<uintptr_t>(alloc) + sizeof_vectorFuncs);

  // Fill vector struct
  vector->capacity = maxSize; vector->count = 0; vector->elementSize = elementSize; vector->flag = vectorFlag;
  va_list args;
  ...
  if (vectorFlag & vector_flagbit_constructor_tapi) {
    vector_element_constructor_func_tapi constructor = va_arg(args, vector_element_constructor_func_tapi);
  ...
  if (vectorFlag & vector_flagbit_destructor_tapi) {
    vector_element_destructor_func_tapi destructor = va_arg(args, vector_element_destructor_func_tapi);
  ...
  if (!resize(vector + 1, initialSize)) { virtual_free(alloc, sizeof_vectorFuncs + sizeof(vector_tapi) + (elementSize * maxSize)); }
  return vector + 1;
}
...
unsigned char erase(void* data, unsigned int elementIndex) {
  vector_tapi* hnd = getVectorTapi(data);
  if (elementIndex >= hnd->count) { return 0; }
  ...
  if (hnd->flag & vector_flagbit_copy_tapi) {
    ...
    for (uint64_t i = elementIndex; i < hnd->count - 1; i++) {
      copyFunc(
        (unsigned char*)data + (hnd->elementSize * (i + 1)),
    ...
    memmove(
      (unsigned char*)data + (hnd->elementSize * elementIndex),
...
allocator_sys_tapi* initialize(void* allocatorSys) {
  ...
  firstPageMemBlockCount = (pagesize - sizeof(supermemoryblock_tapi::superBlockInfo)) / sizeof(supermemoryblock_tapi::memBlock);
...
extern "C" void* initializeAllocator(void* allocatorSys) {
  return initialize(allocatorSys);
}