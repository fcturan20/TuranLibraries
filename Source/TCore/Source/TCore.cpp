...
TCResult LoadPlugin(const char* path, const TCPluginInfo** outInfo) {
  try {
    ...
    auto entryPoint =
      dynalo::get_function<void(const TCServices*, TCPluginInfo*, TCPluginFunctions*)>(
...
TCResult TC_Initialize(const void** outPluginAPI) {
  TCServices* newTC   = new TCServices;
  Context             = new TCContext();
  newTC->GetVersion   = []() -> unsigned int { return TC_PLUGIN_VERSION; };
...
TCResult TC_OnPreShutdown() {
  for (auto& [name, stored] : Context->Plugins) TC->UnloadPlugin(name.c_str());
  return TC_RESULT_SUCCESS;
}
